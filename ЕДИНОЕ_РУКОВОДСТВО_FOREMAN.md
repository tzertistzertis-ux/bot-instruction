...
| Backend-тесты гостевого наблюдателя Foreman | Гость не может создать, изменить, отправить, удалить, сгенерировать, прикрепить, загрузить, переименовать, перенести или запустить backfill даже прямым API-запросом; скачивание остается только в пределах лимитов | 45 автотестов |
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 2000 автопроверок, плюс 3 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
Backend mutation pilot summary: killed=13 survived=0 total=13
```
...
Отдельно добавлен backend mutation pilot Foreman Web: 13 точечных мутантов. Первая часть проверяет Kaspersky и документохранилище: backend не принимает `no_verdict` за чистый файл, не игнорирует HTTP-ошибки Kaspersky, не пропускает серую зону без полного анализа и не отключает серверные запреты для рискованных файлов документохранилища. Вторая часть проверяет гостевого наблюдателя: нельзя снова разрешить гостю создание заявки, примечание в план-графике, запись в документохранилище, обход лимита скачиваний или получение прав автора заявки. Третья часть закрывает свежий security audit Foreman Web: нельзя читать файл через подмененный путь вне хранилища, нельзя вернуть опасное `inline`-скачивание и нельзя убрать browser-защиту `nosniff`.
...
| Гостевой наблюдатель | Просмотр разрешен, запись запрещена: заявки, план-график, документохранилище, файлы, отчеты и прямые API-запросы проверяются на сервере |
