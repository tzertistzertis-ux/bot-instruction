...
Если нужно просто открыть систему и посмотреть, как она устроена, используйте Foreman Web:

[Открыть Foreman Web](https://claim-bot-tzertis.amvera.io/foreman/)
...
### Контроль качества защитных тестов
...
| Зона | Что пытаются<br>сломать | Что должно<br>произойти |
|:---|:---|:---|
...
| План-график и веб-страницы | В поля пытаются подставить строку, которая могла бы выполниться в браузере как код | Тесты должны показать ее только как текст |

Хороший итог выглядит так: все искусственные поломки пойманы, ни одна не “выжила”. В техническом отчете это записывается примерно так:
...
Практическое правило по первым суткам простое: в интервале `0-24ч` бот никому не пишет по цепочке контроля. Он не пишет Оксане Владимировне, не пишет Ларисе Разиповне и не подключает руководителя. В это время бот только смотрит, появилась ли заявка в Google-таблице.

Если строка появилась в Google-таблице до истечения 24 часов, контроль закрывается без писем Оксане и Ларисе. Если строки нет, после 24 часов начинается рабочая цепочка уточнения.
...
Важно: письмо Ларисе Разиповне не отправляется в первые `0-24ч`. Лариса подключается только после окончания первичного 24-часового ожидания и только если Оксана Владимировна дала корректный номер запуска.
...
В обычное и повторное письмо Ларисе Разиповне добавляется короткая хронология и компактная HTML-схема: когда заявка пришла, когда Оксана Владимировна подтвердила запуск, когда бот написал Ларисе и что строка в Google-таблице на момент письма все еще не найдена.

В хронологии обычного письма используется формулировка `отправлен запрос Ларисе Разиповне`, без слова `первый`. Если письмо повторное, повторность видна в теме письма: `Повторно: внести заявку...`. Отдельная вступительная фраза о повторном обращении в теле письма не используется.
...
- первичные 24 часа уже прошли;
- Оксана подтвердила номер запуска;
- номер запуска распознан;
- заявка все еще отсутствует в Google-таблице.

В первые 24 часа отметок `Бот -> Лариса` в схеме быть не должно. Если такая отметка появляется в интервале `0-24ч`, это считается ошибкой визуализации или логики.
...
Для Ларисы есть два рабочих вида писем: обычный запрос внести заявку и повторный запрос. Обычный запрос в хронологии называется просто `отправлен запрос Ларисе Разиповне`; повторность не раздувается лишними фразами в теле письма, а показывается в теме повторного письма.
...
Письмо руководителю не является техническим логом. В нем не показываются внутренние события очереди вроде “письмо поставлено в очередь”. Руководитель видит только рабочую хронологию: когда заявка пришла, когда бот писал Оксане Владимировне, когда писал Ларисе Разиповне, какой ответ был получен и почему заявка все еще требует вмешательства.

В письме также добавляется компактная схема задержки по заявке. Красная строка `Google-таблица` означает главное: заявка пока не заведена, поэтому контроль не закрыт.

Сейчас предусмотрены четыре рабочих варианта подключения руководителя:

| Случай | Когда срабатывает | Как формулируется причина |
| --- | --- | --- |
| 3 | Оксана Владимировна подтвердила запуск, бот написал Ларисе Разиповне, но строка в Google-таблице не появилась | Лариса Разиповна не внесла заявку в таблицу после подтверждения запуска Оксаной Владимировной |
| 4 | Оксана Владимировна не ответила после первого и повторного запроса | Не получено корректное подтверждение от Оксаны Владимировны, что заявка находится в работе |
| 5 | Оксана Владимировна ответила после второго запроса, бот написал Ларисе Разиповне, но строка в Google-таблице не появилась | Лариса Разиповна не внесла заявку в таблицу после подтверждения запуска Оксаной Владимировной |
| 6 | Оксана Владимировна ответила, но без корректного номера запуска вида `С-26-00677` | Оксана Владимировна ответила на запрос, но не указала корректный номер запуска заявки |

Эти четыре варианта закреплены автотестом `test_manager_escalation_covers_all_four_leadership_cases` и входят в обычный быстрый pre-deploy / мега-тест через `release_check.ps1`. Это не `мегатест-2`: проверка локальная, быстрая и не ходит в SMTP, Telegram, Google или Kaspersky.
