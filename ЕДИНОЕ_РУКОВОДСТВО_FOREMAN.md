...
Поэтому в Foreman появился отдельный антивирусный контур. Проверку выполняет внешний API `Kaspersky OpenTIP`. Касперский - российская компания мирового уровня и одна из самых известных компаний в области кибербезопасности; ее решения используют не для красоты, а чтобы заранее отсеивать известные угрозы и подозрительные файлы.
...
| Проверка через Kaspersky OpenTIP | Сначала отправляет цифровой отпечаток файла, а если по нему нельзя получить уверенный ответ, может отправить сам файл во внешний антивирусный API Касперского | Помогает поймать известные угрозы без установки тяжелого антивируса на тариф Amvera |
...
Foreman больше не считает “мутный” ответ безопасным. Зеленая отметка ставится только тогда, когда Kaspersky OpenTIP вернул чистый результат: `Green`, `clean` или смысловой ответ “угрозы не обнаружены”. `Red` - это опасный файл, `Yellow` - подозрительный файл. `Grey`, `not categorized`, `pending`, старая неоднозначная запись `no_verdict` или временная ошибка API не показываются как “чисто”: Foreman отмечает такой файл предупреждением и не притворяется, что проверка полностью завершена.

Если Касперский или локальная проверка считают файл опасным или подозрительным, файл блокируется и не попадает в обычную работу. Если внешний сервис временно не дал окончательный ответ, система фиксирует это в журнале отдельным статусом: такой случай не скрывается “под ковром”.
...
| Полная регрессия Telegram-бота (`full`) | Команды, рендер, golden-проверки, старые сценарии, защита от регрессий, отдельная pytest-матрица реестра запусков, журнал антивирусных проверок, просмотр заблокированных файлов и строгие проверки ответов Kaspersky | 1838 автотестов плюс render/golden-проверки |
...
Итого в последнем успешном прогоне `мегатеста-2` прошло примерно 1872 автопроверки, плюс 2 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
```text
Mutation pilot summary: killed=30 survived=0 total=30
```

Это означает: все выбранные искусственные ошибки были пойманы тестами. Сейчас в этот пилот входят 30 точечных мутантов, из них 12 относятся именно к антивирусному контуру: строгая трактовка `Green/Red/Yellow/Grey/pending`, кодирование русских имен файлов, риск-скачивание, `no_verdict`, `api_unavailable` и свежесть ответа Kaspersky на диагностической странице.
