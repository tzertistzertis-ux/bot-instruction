...
При этом Foreman не верит одному названию файла. Обычный `.txt` остается разрешенным рабочим форматом и проходит антивирусную проверку Kaspersky. Но если программу, архив, картинку или PDF просто переименовать в `.txt`, система должна увидеть несоответствие. То же самое с `.pdf`, `.dwg` и `.dxf`: настоящий документ или чертеж проходит дальше, а переименованная подделка блокируется до отправки в Kaspersky.
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 1996 автопроверок, плюс 3 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
