...
| Backend security audit Foreman Web | XSS при показе имен файлов и антивирусных меток, безопасное скачивание `attachment`, запрет чтения файла вне storage root, security headers веб-страницы | 11 автотестов плюс 1 render-regression-скрипт |
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 2003 автопроверки, плюс 5 отдельных backend regression/smoke-скриптов и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
Backend mutation pilot summary: killed=18 survived=0 total=18
```
