...
**Версия документа: 12.07.2026, редакция 9**

Версионный PDF-файл для передачи и скачивания: `ЕДИНОЕ_РУКОВОДСТВО_FOREMAN_2026-07-12.pdf`

Что изменено в редакции 9: полностью переписано объяснение проверок перед выпуском. «Мегатест» и «Мегатест-2» теперь описаны так, как они действительно устроены после усиления: это два разных независимых контура, каждый со своей задачей, запуском и итоговым отчетом. Объяснены автоматический выбор затронутых компонентов, проверка покрытия и эталонных изображений, безопасная работа только с локальной тестовой PostgreSQL, глубокие проверки параллельных действий, мутаций и нестабильности. Убраны быстро устаревающие счетчики тестов: решение о выпуске принимается по свежему отчету конкретного запуска, а не по красивой цифре из руководства.
...
4. `Web Push` - короткие браузерные уведомления по выбранным событиям Foreman.
...
### Почему в руководстве больше нет “точного общего числа тестов”

Большая цифра выглядит убедительно, но быстро устаревает и может вводить в заблуждение. Один точный сквозной тест иногда защищает пользователя лучше сотни поверхностных проверок, а один новый компонент закономерно меняет общий счетчик.

Поэтому источник истины - свежий отчет конкретного M1 или M2. Он хранит выбранные стадии, точные commit-ы, количество выполненных тестов, покрытие, пропуски и пути к доказательствам. Руководство объясняет смысл защиты, но не подменяет живой отчет числом месячной давности.
