...
|---|---|
| `покажи проверки файлов` | Последние проверки файлов: веб, мобильное приложение, письма и Telegram, если события доступны в журнале |
...
| Полная регрессия Telegram-бота (`full`) | Команды, рендер, golden-проверки, старые сценарии, защита от регрессий, отдельная pytest-матрица реестра запусков, журнал антивирусных проверок и просмотр заблокированных файлов | 1818 автотестов плюс render/golden-проверки |
...
Итого в последнем успешном прогоне `мегатеста-2` прошло примерно 1852 автопроверки, плюс 2 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
| Команда | Что делает |
|---|---|
| `покажи проверки файлов` | Показывает последние проверки файлов: имя, источник, итог проверки и время |
...
| `/file_security_history`, `/file_security`, `/antivirus`, `покажи проверки файлов` | Админ | Последние проверки файлов без содержимого файлов и без секретов |
