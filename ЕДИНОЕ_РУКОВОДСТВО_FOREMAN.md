...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 1979 автопроверок, плюс 3 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
После этих проверок файл проходит антивирусный контур Foreman. Для пользователя это выглядит просто: обычный чистый файл сохраняется как всегда, а файл с опасным, подозрительным или не до конца понятным результатом не попадает в документохранилище как обычный рабочий документ. Сам файл в таком случае не нужно пытаться открыть вручную: достаточно сообщить администратору, что загрузка была заблокирована или требует проверки.
