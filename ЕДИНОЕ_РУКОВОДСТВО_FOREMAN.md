...
**Версия документа: 12.07.2026, редакция 10**
...
На момент усиления контуров измеренное покрытие основных компонентов было таким:

| Компонент | Покрытие всего существующего кода | Покрытие нового и измененного кода |
|:---|:---|:---|
| Claim-bot | около **64%** программных выражений, **67%** строк и **56%** развилок | около **92%** измененных строк и **89%** измененных развилок |
| `foreman-api` | около **75%** выражений, **76%** строк, **63%** развилок и **81%** функций | около **98%** измененных строк и **86%** измененных развилок |
| `office-converter` | около **79%** строк, **73%** развилок и **69%** функций | отдельный порог для измененного кода пока не используется |
| Foreman Mobile | существующие тесты выполняются, но честный общий процент coverage пока не принят | новые Mobile coverage/golden отложены владельцем и не выдаются за готовый результат |

Фразу **“Foreman защищен на 60%” говорить неправильно**. Правильно говорить так: **“в измеренном запуске тесты исполнили около 64% кода claim-bot; для нового и измененного кода проверено около 90% и более”**.

Оставшиеся проценты не означают автоматически “дыру”. Часть кода относится к внешним сервисам, аварийным веткам, запуску программы и системной обвязке. Такие места дополнительно проверяются сквозными, браузерными, визуальными, fault-injection и post-deploy сценариями. Но непокрытый код всё равно остается зоной риска, поэтому M1 запрещает незаметно снижать установленный уровень и требует высокого покрытия нового кода.

Если директору или сотруднику нужно объяснить всё одной фразой, можно сказать так:

**“Перед полным выпуском Foreman выполняет около пяти тысяч автоматических проверок; тесты проходят примерно две трети кода бота и три четверти серверной части, а новый измененный код проверяется примерно на 90% и выше. Самые рискованные правила дополнительно испытываются отдельным Мегатестом-2.”**
