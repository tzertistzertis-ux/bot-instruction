...
Для сотрудников есть две простые команды в Telegram:
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 2007 автопроверок, плюс 5 отдельных backend regression/smoke-скриптов и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
Для спокойной проверки есть две команды:
...
Эти команды не показывают содержимое файлов, пароли или токены. Они нужны, чтобы сотрудник не нервничал: система не молчит, а оставляет понятный след проверки.
