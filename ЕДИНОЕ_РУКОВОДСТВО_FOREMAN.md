...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота, серверная защита гостевого наблюдателя | 430 автотестов |
...
Итого в обычном мега-тесте сейчас проходит примерно 593 автопроверки, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются новые важные сценарии, тестов становится больше.

Внутри регрессионного ядра эти 430 проверок распределяются так:

| Подстадия regression core | Количество |
|---|---:|
| Unit-тесты `foreman-api` | 328 |
...
| Полная регрессия Telegram-бота (`full`) | Команды, рендер, golden-проверки, старые сценарии, защита от регрессий, отдельная pytest-матрица реестра запусков, журнал антивирусных проверок, просмотр заблокированных файлов, строгие проверки ответов Kaspersky, ALARM-письма при вирусах, показ карантинной улики в Foreman и обработчик временных сетевых ошибок Telegram | 1851 автотест плюс render/golden-проверки |
| Backend-тесты антивируса и документохранилища Foreman | Политика `clean/no_verdict/api_unavailable`, полная отправка файла в Kaspersky, серверный запрет скачивания без принятия риска, запрет прикрепления и e-mail-отправки рискованных файлов | 32 автотеста |
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 1973 автопроверки, плюс 3 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
