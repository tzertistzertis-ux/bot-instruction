...
Офисные документы `rtf` и строительные чертежи `dwg`/`dxf` считаются нормальными рабочими форматами Foreman. Это не “странные файлы”, а обычные файлы стройки, старых Word-документов и AutoCAD-процессов. Поэтому Foreman принимает их в документохранилище и Telegram-контуре, пропускает через тот же антивирусный слой и показывает понятную отметку: чисто, риск из-за неполного ответа Kaspersky или опасность.

Даже если на сервере осталась старая переменная со списком форматов для антивирусной отправки, Foreman сохраняет базовый рабочий набор: `pdf`, `doc`, `docx`, `xls`, `xlsx`, `rtf`, `txt`, `jpg`, `jpeg`, `png`, `dwg`, `dxf`. Это защищает от ситуации, когда документ разрешен в интерфейсе, но случайно режется внутренним антивирусным списком.

При этом Foreman не верит одному названию файла. Обычный `.txt` остается разрешенным рабочим форматом и проходит антивирусную проверку Kaspersky. Но если программу, архив, картинку или PDF просто переименовать в `.txt`, система должна увидеть несоответствие. То же самое с основными рабочими форматами `pdf`, `png`, `jpg`, `jpeg`, `rtf`, `doc`, `docx`, `xls`, `xlsx`, `dwg` и `dxf`: настоящий документ, фото или чертеж проходит дальше, а переименованная подделка блокируется до отправки в Kaspersky.
...
Foreman больше не считает “мутный” ответ безопасным. Зеленая отметка ставится только тогда, когда Kaspersky вернул чистый результат: `Green`, `clean` или смысловой ответ “угрозы не обнаружены”. `Red` - это опасный файл, `Yellow` - подозрительный файл. `Grey`, `not categorized`, `pending`, старая неоднозначная запись `no_verdict` или временная ошибка API не показываются как “чисто”: Foreman сохраняет обычный рабочий файл с предупреждающей отметкой и не притворяется, что проверка полностью завершена.
...
Для писем с опасными вложениями добавлена отдельная поддержка сотрудников. Если прораб отправил письмо чат-боту и одновременно поставил в копию менеджеров или руководителей, Foreman формирует ALARM-письмо всем участникам: отправителю, получателям и получателям в копии, кроме самого адреса бота. В письме указывается, кто прислал исходное письмо, когда оно пришло, по какой заявке или заявкам обнаружена проблема, какие файлы не прошли проверку Kaspersky и какое название угрозы передал Касперский, если он его сообщил.
...
Для старых писем и старых внешних вложений предусмотрена одноразовая ретро-проверка. Это не происходит от клика сотрудника по карточке, чтобы не тормозить веб-браузер и не гонять сотни файлов во внешний сервис внезапно. После деплоя бот сам запускает такую проверку в фоне один раз, записывает итог в журнал и ставит служебную отметку `security_backfill_complete.json`, чтобы не повторять старый архив заново при каждом запуске. После этого старые файлы тоже получают понятную отметку в веб-карточке.
...
| Стадия мега-теста | Что проверяет | Примерный объем по последнему успешному прогону |
|---|---|---:|
| Сборка `foreman-api` | Приложение должно собраться без ошибок перед выпуском | Это не тесты, а обязательная сборка |
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота, серверная защита гостевого наблюдателя | 471 автотест |
...
Итого в обычном мега-тесте сейчас проходит примерно 634 автопроверки, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются новые важные сценарии, тестов становится больше.

Внутри регрессионного ядра эти 471 проверка распределяется так:

| Подстадия regression core | Количество |
|---|---:|
| Unit-тесты `foreman-api` | 372 |
| E2E-тесты `foreman-api` | 14 |
| Planning smoke | 28 |
| Real-flow smoke | 1 |
| Request-types smoke | 4 |
| Foreman Mobile compatibility smoke | 3 |
| Claim-bot release check smoke | 52 |
...
| Полная регрессия Telegram-бота (`full`) | Команды, рендер, golden-проверки, старые сценарии, защита от регрессий, отдельная pytest-матрица реестра запусков, журнал антивирусных проверок, просмотр заблокированных файлов, просмотр журнала безопасности Foreman, строгие проверки ответов Kaspersky, ALARM-письма при вирусах, показ карантинной улики в Foreman, запрет браузерных вложений, прием `rtf` и строительных `dwg/dxf`, защита от старой серверной переменной форматов, обработчик временных сетевых ошибок Telegram, PostgreSQL-очередь важных писем, объединение подтверждений прорабам, защита от повторной SMTP-отправки, диагностика зависших входящих писем, ручной разбор непонятных ответов Оксаны/Ларисы, админский просмотр почтовых ожиданий | 1873 автотеста плюс render/golden-проверки |
| Backend-тесты антивируса и документохранилища Foreman | Политика `clean/no_verdict/api_unavailable`, полная отправка файла в Kaspersky, сохранение обычных файлов, `rtf`, `txt` и строительных `dwg/dxf` с риск-меткой при неполном ответе Kaspersky, защита от старой серверной переменной форматов, запрет переименованных файлов под видом `pdf/png/jpg/jpeg/rtf/doc/docx/xls/xlsx/txt/dwg/dxf`, быстрая загрузка нормальных файлов с фоновой проверкой Kaspersky, серверный запрет скачивания без принятия риска, запрет прикрепления и e-mail-отправки рискованных файлов, запрет HTML Smuggling-файлов и маскировки HTML/JavaScript внутри рабочих форматов | 52 автотеста |
...
| Бережная проверка нагрузки живого сайта | Несколько одновременных безопасных запросов без записи в базу: health/readiness, web shell, журнал, план-график, документохранилище и гостевой просмотр | 1 live smoke-прогон; последний результат: 36 запросов, p95 около 706 мс, отказов 0 |
| Единый live security smoke после деплоя | Одна команда проверяет живую Amvera после выпуска: health/readiness, публичную диагностическую страницу, запреты гостевого наблюдателя, обычный вход и бережную readonly-нагрузку. Рабочие заявки и файлы при этом не меняются | 1 объединяющий smoke-прогон; последний результат: endpoints OK, 36 readonly-запросов, p95 около 143 мс, отказов 0 |
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 2036 автопроверок, плюс 6 отдельных backend regression/smoke-скриптов и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
После этих проверок файл проходит антивирусный контур Foreman. Для пользователя это выглядит просто: обычный рабочий файл быстро сохраняется и получает отметку `Проверка Касперского не завершена`, а сама проверка Kaspersky продолжается в фоне. Когда ответ готов, отметка меняется на зеленую `Kaspersky: угроз не найдено` или на предупреждение/опасность. Если Kaspersky не дал окончательный зеленый ответ, но и не сообщил об угрозе, файл не теряется: Foreman сохраняет его с предупреждающей отметкой `РИСК`, а скачивание требует осознанного подтверждения. Это относится и к `rtf`, `txt`, и к строительным чертежам `dwg/dxf`: они разрешены как рабочие форматы, но все равно получают антивирусную отметку. Если обычный текстовый, архивный, HTML/SVG или программный файл просто переименовали в `.pdf`, `.png`, `.jpg`, `.rtf`, `.doc`, `.docx`, `.xls`, `.xlsx`, `.dwg` или `.dxf`, Foreman не принимает его как настоящий документ, фото или чертеж. Если программу, архив, PDF или картинку переименовали в `.txt`, Foreman не принимает такой файл как обычный текст. Если файл опасный, подозрительный или запрещенный по типу, загрузка блокируется сразу и не сохраняется.
...
В этом ответном письме бот теперь объясняет новый порядок контроля:

- бот не просто пишет “файл получен”, а сообщает, что дальше проконтролирует запуск заявки в работу;
...
После этого бот больше не должен продолжать письма по этой заявке в рамках контроля “не внесена в таблицу”.
