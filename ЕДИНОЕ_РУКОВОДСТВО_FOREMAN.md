...
| `Security/audit-тесты` | Проверяют защитный контур: файл должен пройти проверку, а журнал должен записать итог без содержимого файла, паролей и токенов | Попадание опасного файла в обычное хранилище, утечку секретов в диагностике, отсутствие понятного следа проверки |
...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота, серверная защита гостевого наблюдателя | 419 автотестов |
...
Итого в обычном мега-тесте сейчас проходит примерно 582 автопроверки, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются новые важные сценарии, тестов становится больше.

Внутри регрессионного ядра эти 419 проверок распределяются так:

| Подстадия regression core | Количество |
|---|---:|
| Unit-тесты `foreman-api` | 317 |
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 1962 автопроверки, плюс 2 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
Backend mutation pilot summary: killed=10 survived=0 total=10
```
...
| Антивирусный контур файлов | Проверка безопасного сохранения, строгая трактовка ответов Kaspersky, аудит-журнал без секретов, Telegram-просмотр последних проверок и заблокированных файлов, ALARM-письма участникам опасного письма, показ карантинной улики в Foreman, серверный risk-gate документохранилища |
