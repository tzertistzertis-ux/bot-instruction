...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота, серверная защита гостевого наблюдателя, базовая проверка proxy-allowlist | около 815+ автотестов в текущем составе smoke/regression-контуров |
...
Итого в обычном мега-тесте проходит примерно 1000+ автопроверок, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются важные сценарии, тестов становится больше. Самым надежным источником точного числа всегда остается свежий вывод `pre_deploy_all.ps1`, `release_check.ps1`, `npm test` и `flutter test`.

Внутри регрессионного ядра проверки распределяются так:

| Подстадия<br>regression core | Количество |
|:---|:---|
| Jest-тесты `foreman-api` | 518 |
| Отдельные E2E/smoke-прогоны `foreman-api` | 14+ |
| Planning smoke | 28 |
| Real-flow smoke | 1 |
...
| Стадия<br>мега-теста | Что<br>проверяет | Примерный<br>объем |
|:---|:---|:---|
| Browser-тесты Foreman Web UI Playwright | Реальные действия в браузере: заявки, e-mail overlay, профиль, push-настройки, план-график, retry при ошибке расписания | 15 браузерных автотестов |
...
| Backend-тесты bot-defense/honeypot | Скрытое поле входа не видно человеку, один подозрительный случай только журналируется, временная блокировка включается осторожно и только при повторном автоматическом поведении или вместе с частыми ошибками входа, админские настройки и снятие блокировки недоступны обычному пользователю, а поддельные IP-заголовки не используются как доверенный адрес клиента | 11 auth-security e2e-тестов плюс целевые unit-тесты нормализации IP и bot-defense |
...
Итого в `мегатесте-2` в текущем стандартном профиле проходит примерно 2100+ автопроверок, плюс отдельные backend/proxy regression/smoke-скрипты и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
| **Foreman сейчас** | **720-780** | Веб, мобильное приложение и Telegram-бот имеют несколько рабочих слоев защиты: серверные роли, гостевые ограничения, proxy-allowlist, антивирусный контур, контроль опасных форматов, журналы безопасности, осторожный honeypot/bot-defense на входе, HttpOnly cookie-сессию для веба, защиту rate-limit от простой подмены forwarding-заголовков, управляемые Web Push-оповещения, регрессии, мегатест и мегатест-2. Диапазон повышен умеренно: cookie-сессия и защита от IP-spoofing закрыли два практичных риска, но не заменили независимый пентест, WAF/anti-DDoS, SSO/MFA, формальный SOC/мониторинг и регулярный внешний security-аудит |
| Зрелая строительная SaaS/ECM-система | 820-900 | Обычно добавлены отдельные security-процессы, корпоративный мониторинг, внешние проверки, SSO/MFA, договорные SLA и регулярные аудиты |
...
|:---|:---|:---|:---|
| Foreman Web в браузере | 730-800 | Авторизация, серверные роли, защита гостя, лимиты скачивания, журнал безопасности, proxy-allowlist, honeypot/bot-defense на входе, HttpOnly cookie-сессия вместо хранения основного веб-токена в `localStorage`, нормализация IP для rate-limit, отключение наружного `X-Powered-By: Express`, антивирусный контур, запрет опасных вложений, профильные Web Push-подписки, админские правила push-событий, UI/visual-регрессии | Ужесточить CSP без `unsafe-inline`, добавить внешний WAF/anti-DDoS, SSO/MFA и независимый пентест |
...
| Система Foreman в целом | 720-780 | Защита не держится на одной кнопке: права проверяются сервером, файлы проходят антивирусный контур, лишние proxy-маршруты режутся до backend, веб-вход переведен на HttpOnly cookie, вход дополнительно прикрыт honeypot/bot-defense, rate-limit не доверяет подставленным forwarding-заголовкам, важные события могут дублироваться управляемым Web Push, изменения проверяются тестовыми контурами | Внешний пентест, WAF/anti-DDoS, SSO/MFA, формальная карта реагирования на инциденты, регулярная проверка зависимостей и секретов |
...
- безопасность закреплена тестами, мегатестом, мегатестом-2 и live-smoke после деплоя;
...
Честно: honeypot не делает сайт “невзламываемым” и не заменяет MFA, внешний WAF/anti-DDoS, независимый пентест и круглосуточный мониторинг. Но для текущего масштаба Foreman это полезный слой: он снижает риск простого автоматического подбора и дает администратору ранние признаки, что форму входа пытается использовать программа.
...
### 1.15.5 Как создать пользователя аккуратно
