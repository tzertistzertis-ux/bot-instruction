...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота, серверная защита гостевого наблюдателя | 467 автотестов |
...
Итого в обычном мега-тесте сейчас проходит примерно 630 автопроверок, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются новые важные сценарии, тестов становится больше.

Внутри регрессионного ядра эти 467 проверок распределяются так:

| Подстадия regression core | Количество |
|---|---:|
| Unit-тесты `foreman-api` | 365 |
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 2017 автопроверок, плюс 5 отдельных backend regression/smoke-скриптов и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
Backend mutation pilot summary: killed=22 survived=0 total=22
```
