...
**Версия документа: 15.06.2026, редакция 4**

Версионный PDF-файл для передачи и скачивания: `ЕДИНОЕ_РУКОВОДСТВО_FOREMAN_2026-06-15.pdf`

Что изменено в редакции 4: для Foreman Mobile зафиксирован релиз `0.2.107+6108`, нормальный Android-сценарий обновления APK через проверенный файл и системный установщик, мягкий anti-tamper контур root/debug/Frida, блокировка явных debug/Frida-сценариев мобильного входа, предупреждение по root-сигналам без грубого автоматического бана, а также обновлены честные оценки безопасности и тестовые контуры мега-теста/мегатеста-2.
...
| Стадия<br>мега-теста | Что<br>проверяет | Примерный<br>объем |
|:---|:---|:---|
| Browser-тесты Foreman Web UI Playwright | Реальные действия в браузере: заявки, e-mail overlay, профиль, push-настройки, безопасное подключение MFA, доверенные устройства, управление браузерными и мобильными профилями, план-график, retry при ошибке расписания | 19 браузерных автотестов |
...
| Foreman Mobile release/security regression | Мобильный контур, который специально не дает незаметно сломать безопасное обновление APK, подпись `app-update.json.sig`, SHA-256 APK, запуск системного Android-установщика, production identity, obfuscation, отсутствие лишних разрешений, постоянный `installationId`, безопасный `deviceProfile` и anti-tamper сигналы root/debug/Frida | 23 мобильных автотеста |

Итого в `мегатесте-2` в текущем стандартном профиле проходит примерно 2175+ автопроверок, плюс отдельные backend/proxy regression/smoke-скрипты и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
| **Foreman сейчас** | **750-810** | Веб, мобильное приложение и Telegram-бот имеют несколько рабочих слоев защиты: серверные роли, гостевые ограничения, proxy-allowlist, антивирусный контур, контроль опасных форматов, журналы безопасности, осторожный honeypot/bot-defense на входе, HttpOnly cookie-сессию для веба, защиту rate-limit от простой подмены forwarding-заголовков, TOTP/MFA для веб-входа и мобильного админского входа, расширенный цифровой слепок браузера, доверенные устройства и наблюдение риск-сигналов входа в audit-контуре, управляемые Web Push-оповещения, мобильную APK-сборку с obfuscation, проверку подписи манифеста обновления и SHA-256 APK, корректный Android install-flow обновления, anti-tamper сигналы root/debug/Frida, управление доверенными мобильными устройствами с отзывом сессий экземпляра Foreman Mobile на конкретном смартфоне, регрессии, мегатест и мегатест-2. Диапазон дан консервативно: оценка опирается только на реально внедренные меры, а не на обещания будущего аудита. Foreman уже заметно сильнее обычного внутреннего сайта, но еще не является зрелой промышленной SaaS/ECM-платформой |
...
- мобильное обновление не бросает сотрудника искать APK в памяти телефона: Foreman скачивает проверенный файл во внутреннее временное хранилище, открывает системный Android-установщик и при необходимости переводит пользователя в настройку разрешения установки именно для Foreman;
...
5. При необходимости сменить пароль или отключить пользователя полностью.
...
Текущая версия в исходниках: `0.2.107+6108`.
