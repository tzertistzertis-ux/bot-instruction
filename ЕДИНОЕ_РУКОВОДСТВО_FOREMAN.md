...
**Версия документа: 10.06.2026**

Версионный PDF-файл для передачи и скачивания: `ЕДИНОЕ_РУКОВОДСТВО_FOREMAN_2026-06-10.pdf`
...
Итого в обычном мега-тесте проходит примерно 1060+ автопроверок, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются важные сценарии, тестов становится больше. Самым надежным источником точного числа всегда остается свежий вывод `pre_deploy_all.ps1`, `release_check.ps1`, `npm test` и `flutter test`.

Внутри регрессионного ядра проверки распределяются так:

| Подстадия<br>regression core | Количество |
|:---|:---|
| Jest-тесты `foreman-api` | 556 |
...
| Стадия<br>мега-теста | Что<br>проверяет | Примерный<br>объем |
|:---|:---|:---|
| Browser-тесты Foreman Web UI Playwright | Реальные действия в браузере: заявки, e-mail overlay, профиль, push-настройки, безопасное подключение MFA, цифровые слепки браузеров, управление доверенными устройствами, план-график, retry при ошибке расписания | 18 браузерных автотестов |
...
| Единый live security smoke после деплоя | Одна команда проверяет живую Amvera после выпуска: health/readiness, публичную диагностическую страницу, запреты гостевого наблюдателя, обычный вход при наличии актуальных smoke-учетных данных и бережную readonly-нагрузку. Рабочие заявки и файлы при этом не меняются | 1 объединяющий smoke-прогон. Последний деплой: endpoints и guest security OK; valid-login smoke требует актуальные smoke-учетные данные |
...
Хороший итог выглядит так: все искусственные поломки пойманы, ни одна не “выжила”. В техническом отчете это записывается примерно так (актуальный ориентир на 13.06.2026):

```text
Mutation pilot summary: killed=40 survived=0 total=40
Backend mutation pilot summary: killed=50 survived=0 total=50
```
...
Важно: оценка тестового контура, например `865 / 1000`, **не является официальным сертификатом, ГОСТ-аудитом, ISO-аудитом или внешним заключением**. В мире тестирования нет одной обязательной государственной или международной формулы, которая автоматически превращает набор тестов в точную оценку по 1000-балльной шкале.

Это внутренняя управленческая риск-оценка Foreman. Она нужна, чтобы коротко объяснить руководству, насколько тесты реально защищают рабочую систему от регрессий.

Оценка строится не на ощущениях, а на проверяемых фактах:
...
| Регрессии и мутации | Есть ли тесты, которые ловят уже известные опасные поломки, и проходят ли targeted mutation checks |
| Интеграционные проверки | Проверяются ли не только отдельные функции, но и связка backend, web, mobile, bot, база, файлы, письма |
...
| Место в процессе | Стоят ли проверки перед деплоем с домашнего ПК через `pre_deploy_all.ps1` и `risk_assurance_all.ps1` |
| Документация | Понятно ли, что запускать и когда деплой запрещен |

Число `865 / 1000` означает: тестовый контур Foreman сильный для текущего масштаба проекта, но не абсолютный. Баллы снимаются не потому, что тесты “плохие”, а потому что часть вещей нельзя полностью заменить локальной автоматикой: Amvera live-состояние, реальная доставка Gmail/SMTP, Telegram API, Google-доступы и установка APK на настоящий Android.

Как перепроверить основание оценки:

```powershell
cd "E:\файлы к чат-боту\Бот собранный модулями_MAX"
powershell -ExecutionPolicy Bypass -File .\scripts\pre_deploy_all.ps1 -List
powershell -ExecutionPolicy Bypass -File .\scripts\risk_assurance_all.ps1 -List
npm run test:risk:mutation:check
```

Подробная фактура лежит в техническом отчете:

```text
docs\TEST_AUDIT_FACT_2026-06-13.md
```
...
| **Foreman сейчас** | **745-805** | Веб, мобильное приложение и Telegram-бот имеют несколько рабочих слоев защиты: серверные роли, гостевые ограничения, proxy-allowlist, антивирусный контур, контроль опасных форматов, журналы безопасности, осторожный honeypot/bot-defense на входе, HttpOnly cookie-сессию для веба, защиту rate-limit от простой подмены forwarding-заголовков, TOTP/MFA для веб-входа, расширенный цифровой слепок браузера и известные устройства в audit-контуре, наблюдение риск-сигналов входа, управляемые Web Push-оповещения, регрессии, мегатест и мегатест-2. Диапазон повышен честно и умеренно: оценка опирается только на реально внедренные меры, а не на обещания будущего аудита. Foreman уже заметно сильнее обычного внутреннего сайта, но еще не является зрелой промышленной SaaS/ECM-платформой |
...
|:---|:---|:---|:---|
| Foreman Web в браузере | 755-805 | Авторизация, серверные роли, защита гостя, лимиты скачивания, журнал безопасности, proxy-allowlist, honeypot/bot-defense на входе, HttpOnly cookie-сессия вместо хранения основного веб-токена в `localStorage`, нормализация IP для rate-limit, TOTP/MFA с QR-кодом и резервными кодами, выборочная админская галочка `Требовать MFA`, предупреждение перед подключением MFA, расширенный цифровой слепок браузера, известные устройства по hash, админский просмотр и управление доверенными устройствами, audit-наблюдения риска входа, отключение наружного `X-Powered-By: Express`, антивирусный контур, запрет опасных вложений, профильные Web Push-подписки, админские правила push-событий, UI/visual-регрессии | Ужесточить CSP без `unsafe-inline`, добавить внешний WAF/anti-DDoS, SSO, включить risk-based step-up после периода наблюдения и провести независимый пентест |
