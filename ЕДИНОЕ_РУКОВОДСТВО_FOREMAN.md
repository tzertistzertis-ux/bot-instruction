...
Foreman постепенно перестает быть просто полкой для файлов. Он учится читать входящие заявки до того, как человек открыл документ вручную: находит номер заявки, объект, корпус, секцию, строки изделий, размеры, формулы стеклопакетов, количество и подозрительные места. Если в заявке не видно высоту, пропущена формула или количество написано странно, система может заранее подсветить это в письме и в служебных данных.

Слово `OCR` в технических текстах означает "распознавание текста". Проще говоря, компьютер смотрит на файл почти как на картинку и пытается понять, какие слова, числа и таблицы в нем написаны. В Foreman эта работа не делается "на глазок" внутри одного скрипта. Для изображений и сканов используется Яндекс Vision OCR - сервис компьютерного зрения Яндекса для распознавания и извлечения текста из изображений и документов. Foreman через защищенное API-подключение отправляет туда файл или страницу файла, получает обратно распознанный текст и координаты найденных фрагментов, а затем уже сам разбирает результат своим parser-ом.

Важно разделять две части работы:

| Этап | Кто выполняет | Что получается |
|:---|:---|:---|
| Распознавание текста | Яндекс Vision OCR, когда файл является картинкой, сканом или PDF-страницей без надежного текстового слоя | Сырой распознанный текст и фрагменты документа |
| Разбор заявки | Foreman parser | Номер заявки, объект, строки изделий, размеры, формулы, количество и замечания к строкам |
| Приведение к рабочему виду | Foreman backend | Нормализованные номера, единый формат размеров, признаки качества, записи в базе и понятный блок для письма |

Главное правило осталось прежним: сначала безопасность, потом чтение. Если файл не прошел защитный контур и не получил чистый вердикт, Foreman не должен спокойно отправлять его на распознавание как обычный рабочий документ.

Как обрабатываются форматы сейчас:

| Формат | Как идет обработка | Статус |
|:---|:---|:---|
| `docx` | Foreman читает структуру Word-документа своим parser-ом, без внешнего распознавания, если документ нормально собран | Готово для controlled production, сложная верстка все равно требует ручной сверки |
| `xlsx` | Foreman читает таблицы своим parser-ом | Поддержано, rollout нужен аккуратный, на реальных рабочих шаблонах |
| `doc` | Сначала отдельный `office-converter` переводит старый Word-файл в `docx`, затем Foreman читает его как современный документ | Controlled pilot после live smoke converter-а |
...
| `jpg`, `jpeg`, `png` | После проверки безопасности файл может идти в Яндекс Vision OCR, затем результат разбирает Foreman parser | Canary/controlled режим: фото с объекта часто бывают шумными |
...
`office-converter` нужен для старых форматов `.doc` и `.xls`. Эти файлы живут в старом мире Office, LibreOffice-конвертации и возможных зависаний, поэтому converter вынесен в отдельный сервис. Он не получает доступ к базе Foreman, не хранит файлы постоянно и работает по токену. Его задача узкая: принять старый Office-файл, вернуть современный `.docx` или `.xlsx`, а дальше Foreman разбирает документ уже обычным путем. Если converter недоступен, backend не должен падать: файл остается для ручной обработки или безопасного повторного запуска позже.

Результат чтения хранится в PostgreSQL. Полная картина остается в `request_content_extractions.canonical_json`, а первый аналитический слой уже разложен по таблицам:
...
Это уже не просто "текст из картинки". Foreman превращает распознанный документ в рабочие данные, которые можно проверять, сравнивать и показывать человеку. Но это еще не финальная промышленная аналитика. Остаются важные задачи: backfill старых распознаваний, расширение item-модели под реальные отчеты, отдельный report/UI для руководителя и более плотные row-level quality tests на неоднозначные размеры, мусорные формулы и странные количества.

Практическая польза для прораба и офиса:

- если в заявке пропущена высота, количество или формула стеклопакета, Foreman может добавить понятный блок замечаний в подтверждающее письмо;
- если в одном письме по одной заявке пришли два файла, система может объединить дополняющие данные, а не считать каждый файл отдельной истиной;
- если имя файла или тема письма ошиблись, а внутри документа найден нормальный номер заявки, главным становится внутренний номер из содержимого;
- если данные конфликтуют, Foreman должен предупредить, а не молча выбрать случайную версию.

Письма прорабам устроены бережно: замечания по распознаванию добавляются отдельным блоком и группируются по заявкам. Заявка без проблем не попадает в красный блок "за компанию". Технические поля вроде `items[0].factoryMarking` не уходят человеку; в письме пишется нормальный рабочий язык: "заводская маркировка", "ширина", "высота", "формула стеклопакета", "количество".

Администратору важно помнить: распознавание файлов - это усиление контроля, а не автоматический судья. В production его правильно включать через feature flags, малые лимиты, dry-run для рискованных исходящих действий и canary-набор реальных файлов. Сырой OCR-текст и сырой ответ провайдера по умолчанию не должны храниться без отдельной осознанной причины: в рабочих документах может быть лишняя информация, которой не место в логах.
...
| OCR, office-converter и structured extraction | Native parser для `docx/xlsx`, controlled path для `doc/xls` через `office-converter`, OCR safety flags, dry-run, grouped OCR remarks in foreman emails, multi-attachment merge, canonical claim number, row-level quality issues, analytics tables `ocr_extraction_claims/items/quality_issues`, PostgreSQL integration gate и targeted OCR mutation checks |
...
| Security Monitoring и тревоги | События пишутся в PostgreSQL, важные случаи превращаются в тревоги, повторы дедуплицируются, Telegram не засыпает администратора одинаковыми сообщениями, ночной вход считается по `Europe/Moscow`, ежедневный отчет собирается за предыдущий московский день, а закрытие тревоги оставляет след `alert_closed` |
