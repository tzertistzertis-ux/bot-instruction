...
| Foreman Mobile | 720-770 | Актуальная APK-сборка не позволяет в production переключить приложение на чужой сервер, запрещает cleartext HTTP, очищает неподходящий сохраненный API-адрес, убирает дефолтный сервисный PIN | Перейти с debug-signing и `com.example...` на постоянную production-подпись и нормальный applicationId, затем провести мобильный security-review |
...
Главный вывод для руководства: Foreman уже не выглядит как “любительский сайт с паролем”. По текущему набору защит он ближе к усиленному рабочему сервису малого/среднего бизнеса. Но он еще не равен крупным промышленным платформам: там безопасность держится не только на коде, а на постоянной команде, мониторинге, внешних аудитах, юридических процедурах, bug bounty и отдельной инфраструктуре защиты.
...
- мобильная APK-сборка больше не должна уводить пользователя на чужой сервер в production.
