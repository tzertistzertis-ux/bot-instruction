...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота, серверная защита гостевого наблюдателя, базовая проверка proxy-allowlist | около 560+ автотестов в текущем составе smoke/regression-контуров |
| Python-тесты Telegram/Foreman request-layer | Внутренний слой заявок, совместимость старых форматов, безопасность владельца, delivery, artifacts, upsert, HTTP routes и proxy-allowlist | 101+ автотест |
...
|:---|:---|
| Jest-тесты `foreman-api` | около 461 |
...
Итого в `мегатесте-2` проходит примерно 2037+ автопроверок, плюс 7 отдельных backend/proxy regression/smoke-скриптов и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
| 300-550 | Базовая защита: есть логин и пароль, но мало серверных запретов, журналов и тестов |
...
| Обычный небольшой сайт компании | 350-550 | Есть форма входа и HTTPS, но мало серверных запретов, мало тестов, слабый контроль файлов |
...
| **Foreman сейчас** | **750-810** | Веб, мобильное приложение и Telegram-бот имеют несколько слоев защиты: серверные роли, гостевые ограничения, proxy-allowlist, антивирусный контур, запрет опасных форматов, журналы безопасности, регрессии, мегатест и мегатест-2 |
...
|:---|:---|:---|:---|
| Foreman Web в браузере | 760-820 | Авторизация, серверные роли, защита гостя, лимиты скачивания, журнал безопасности, proxy-allowlist, антивирусный контур, запрет опасных вложений, тесты регрессии | Ужесточить CSP без `unsafe-inline`, добавить внешний WAF/anti-DDoS и независимый пентест |
| Telegram-бот и claim-bot | 760-830 | Проверка ролей, безопасные команды, quarantine/risk-gate файлов, ALARM-письма при вирусах, proxy-allowlist, диагностика, тесты полного regression-контура | Добавить более формальный мониторинг алертов и регулярный внешний аудит |
| Foreman Mobile | 720-770 | Актуальная APK-сборка в production работает только с утвержденным сервером Foreman, запрещает cleartext HTTP, очищает неподходящий сохраненный API-адрес и не использует дефолтный сервисный PIN | Перейти с debug-signing и `com.example...` на постоянную production-подпись и нормальный applicationId, затем провести мобильный security-review |
| Система Foreman в целом | 750-810 | Защита не держится на одной кнопке: права проверяются сервером, файлы проходят антивирусный контур, лишние proxy-маршруты режутся до backend, изменения проверяются тестовыми контурами | Внешний пентест, WAF/anti-DDoS, формальная карта реагирования на инциденты, регулярная проверка зависимостей и секретов |

Главный вывод для руководства: Foreman не является “любительским сайтом с паролем”. По текущему набору защит он ближе к верхней границе усиленного рабочего сервиса малого/среднего бизнеса. Но он еще не входит в лигу крупных промышленных платформ: там безопасность держится не только на коде, но и на постоянной команде экспертов, которая круглосуточно анализирует события в IT-инфраструктуре, выявляет угрозы и оперативно реагирует на киберинциденты; на внешних аудитах; на юридических процедурах; на bug bounty (программе, в рамках которой компании платят независимым исследователям, багхантерам или “белым хакерам”, за обнаружение и подтверждение уязвимостей в продуктах, сервисах или инфраструктуре); и на отдельной инфраструктуре защиты, включая собственные дата-центры (ЦОД), физическую охрану серверных помещений, охраняемый периметр и контроль доступа.

Почему оценка не 950+:

- нет независимого внешнего пентеста с официальным отчетом;
- нет отдельного WAF/anti-DDoS уровня Cloudflare/enterprise-класса перед всеми публичными входами;
- мобильное приложение еще нужно перевести на постоянную production-подпись и нормальный applicationId;
- для браузера полезно дополнительно ужесточить Content Security Policy;
- нет круглосуточного SOC/дежурства, как у крупных технологических платформ.

Почему оценка уже высокая для текущего масштаба:
