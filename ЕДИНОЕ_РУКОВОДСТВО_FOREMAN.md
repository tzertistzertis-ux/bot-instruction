...
| Раздел | Что внутри |
|---|---|
| `Раздел 0. Короткая карта системы` | Как связаны веб, мобильное приложение, Telegram-бот, база данных и проверки перед выпуском |
...
## 0.5 Как Foreman проверяется перед выпуском в работу
...
| `Contract-тесты` | Проверка формы данных: какие поля есть у заявки, карточки, статуса, вложения, ответа API | Поломку связки веба, мобильного приложения и backend |
| `Integration/e2e-тесты` | Проверка нескольких частей вместе: backend, маршруты, заявки, план-график, письма, файлы | Ошибки на стыке модулей |
...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота | 351 автотест |
...
Итого в обычном мега-тесте сейчас проходит примерно 514 автопроверок, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются новые важные сценарии, тестов становится больше.
...
| Стадия `мегатеста-2` | Что проверяет | Примерный объем по последнему успешному прогону |
|---|---|---:|
| Foreman Web UI Playwright | Реальные действия в браузере: заявки, e-mail overlay, план-график, retry при ошибке расписания | 10 браузерных автотестов |
| Visual regression | Внешний вид важных экранов, чтобы после правок не съехали карточки, кнопки, ошибки и мобильный вид | 2 визуальных автотеста |
| Python fault injection | Отказоустойчивость: e-mail spool, отключенный пользователь, восстановление пароля, rollback файлов | 6 автотестов |
| Boundary/negative tests | Опасные границы и плохие входные данные: файлы, лимиты тела запроса, e-mail recipients, path traversal | 11 автотестов |
| Bot full regression | Полная проверка Telegram-бота: команды, рендер, golden-проверки, старые сценарии, защита от регрессий | 1605 автотестов плюс render/golden-проверки |
| PostgreSQL integration | Локальная тестовая база: startup migrations, readiness, login, несовместимая схема | 3 интеграционных автотеста |
| Backend extra regressions | Дополнительные проверки хранения документов, обновления данных, input/render-логики | 2 отдельных regression-скрипта |

Итого в последнем успешном прогоне `мегатеста-2` прошло примерно 1637 автопроверок, плюс 2 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.

Какие специальные типы проверок там используются:

| Тип проверки | Простыми словами | Что именно проверяем в Foreman |
|---|---|---|
| Boundary tests, или проверки границ | Проверяем край допустимого: ровно столько, сколько можно, и сразу чуть больше, чем можно | Размеры файлов и тела запроса, лимиты e-mail получателей, опасные границы путей к файлам, чтобы система не пропустила лишнее |
| Negative tests, или проверки плохих данных | Специально даем системе неправильные данные и смотрим, откажет ли она спокойно, без падения | Плохие файлы, неверные пути, пустые или неправильные e-mail адреса, слишком большие запросы, попытки обойти правила |
| Contract tests, или проверки договора между частями системы | Проверяем, что разные части программы понимают друг друга одинаково | Backend, веб-браузер, мобильное приложение, Telegram-бот и PostgreSQL должны ожидать одни и те же поля, статусы, маршруты и форматы ответов |
| Fault injection tests, или проверки отказов | Искусственно создаем неприятность и смотрим, не теряет ли система данные | Ошибка отправки письма, отказ сохранения файла, отключенный пользователь со старым токеном, повтор recovery-кода, rollback при сбое metadata-store |
| Visual regression tests, или проверка внешнего вида | Сравниваем важные экраны с эталоном, чтобы после правки интерфейс не "поехал" | План-график, карточки, ошибки загрузки, кнопки, мобильный вид и важные состояния веб-интерфейса |

Для обычного сотрудника смысл простой: эти проверки нужны не ради отчетности. Они заранее ловят ситуации, когда программа вроде бы открывается, но где-то на краю сценария может потерять файл, неправильно отказать пользователю, сломать связь между телефоном и сайтом или показать кривой экран.

Для простой проверки перед крупным релизом исполнитель должен использовать такой порядок:

```powershell
cd "E:\файлы к чат-боту\Бот собранный модулями_MAX"
.\scripts\pre_deploy_all.ps1
npm run postgres:local:start
. E:\postgresql-local\foreman-test-env.ps1
npm run test:risk:assurance
```

Важно: локальная PostgreSQL для `мегатеста-2` - это не рабочая база Amvera. Она специально создана на компьютере как одноразовая тестовая база. Нельзя подставлять рабочую `DATABASE_URL` в `FOREMAN_TEST_DATABASE_URL`, потому что PostgreSQL integration очищает тестовую схему.

Если изменения касались входа, пользователей, паролей, readiness, PostgreSQL или схемы базы, стадия `RISK: PostgreSQL integration` должна быть `OK`, а не `SKIP`.
...
```text
Mutation pilot summary: killed=7 survived=0 total=7
```

Это означает: все выбранные искусственные ошибки были пойманы тестами.

Внутри регрессионного ядра эти 351 проверки распределяются так:

| Подстадия regression core | Количество |
|---|---:|
| Unit-тесты `foreman-api` | 270 |
| E2E-тесты `foreman-api` | 13 |
| Planning smoke | 24 |
| Real-flow smoke | 1 |
...
Если хотя бы одна стадия красная, деплой запрещен. То есть изменение не должно попасть пользователям просто потому, что “на глаз вроде работает”.
