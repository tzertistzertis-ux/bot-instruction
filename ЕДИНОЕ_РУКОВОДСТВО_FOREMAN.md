...
| Блок `мегатеста-2` | Зачем он нужен |
|---|---|
| Foreman Web UI Playwright | Проверяет реальные клики в браузере, карточки, e-mail overlay, план-график, retry при ошибке расписания |
| Visual regression | Ловит неожиданный визуальный сдвиг важных экранов |
| Python fault injection | Проверяет отказоустойчивость: e-mail spool, disabled-user token, password recovery, rollback файлов |
| Boundary/negative tests | Проверяет опасные границы: файлы, лимиты тела запроса, e-mail recipients, path traversal |
| Bot full regression | Глубже проверяет Telegram-бота перед крупным релизом |
| PostgreSQL integration | На локальной тестовой базе проверяет startup migrations, readiness, login и несовместимую схему |
| Backend extra regressions | Проверяет дополнительные сценарии хранения, рендера и обновления UI |
