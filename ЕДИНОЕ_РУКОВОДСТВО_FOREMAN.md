...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота | 374 автотеста |
...
Итого в обычном мега-тесте сейчас проходит примерно 537 автопроверок, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются новые важные сценарии, тестов становится больше.

Внутри регрессионного ядра эти 374 проверки распределяются так:
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 1914 автопроверок, плюс 2 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
