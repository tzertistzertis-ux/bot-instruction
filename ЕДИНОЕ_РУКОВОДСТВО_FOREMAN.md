...
Главная мысль простая: тесты не “для программистов”. Они нужны, чтобы пользователь не узнал первым, что сломался вход, журнал, карточка заявки, план-график, письмо или мобильное приложение.

### Что именно проверяется

| Вид проверки | Простыми словами | Что ловит |
|---|---|---|
| `Smoke-тесты` | Быстрый запуск главных сценариев: сайт открывается, вход работает, заявка создается, журнал отвечает, PDF/письмо формируются в тестовом режиме | Грубую поломку после правки |
| `Регрессионные тесты` | Проверка, что старое рабочее поведение не сломалось из-за новой функции | Случаи “вчера работало, сегодня перестало” |
...
| `Мутационное тестирование` | В код специально временно вносят маленькую ошибку и смотрят, поймают ли ее тесты | Пустые или слишком слабые тесты |
