...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 1982 автопроверки, плюс 3 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
После этих проверок файл проходит антивирусный контур Foreman. Для пользователя это выглядит просто: обычный чистый файл сохраняется как всегда. Если Kaspersky не дал окончательный зеленый ответ, но и не сообщил об угрозе, файл не теряется: Foreman сохраняет его с предупреждающей отметкой `РИСК`, а скачивание требует осознанного подтверждения. Если файл опасный, подозрительный или запрещенный по типу, загрузка блокируется.
