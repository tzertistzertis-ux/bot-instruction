...
**Версия документа: 14.06.2026, редакция 2**

Версионный PDF-файл для передачи и скачивания: `ЕДИНОЕ_РУКОВОДСТВО_FOREMAN_2026-06-14.pdf`

Что изменено в редакции 2: для Foreman Mobile зафиксированы APK obfuscation, подписанный манифест обновления `app-update.json.sig`, проверка подписи манифеста перед SHA-256 скачанного APK, версия мобильного релиза `0.2.104+6105`, а также обновлены честные оценки безопасности и количество актуальных тестов.
...
| Тесты мобильного приложения | Экраны, маршруты, кэш заявок, заявки на заполнения/расходники, защищенное хранение офлайн-черновиков и очереди отправки, проверка безопасного обновления APK, подпись манифеста обновления, мобильный MFA-вход и совместимость с backend | 86 автотестов |

Итого в обычном мега-тесте проходит примерно 1125+ автопроверок, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются важные сценарии, тестов становится больше. Самым надежным источником точного числа всегда остается свежий вывод `pre_deploy_all.ps1`, `release_check.ps1`, `npm test` и `flutter test`.
...
| Стадия<br>мега-теста | Что<br>проверяет | Примерный<br>объем |
|:---|:---|:---|
| Browser-тесты Foreman Web UI Playwright | Реальные действия в браузере: заявки, e-mail overlay, профиль, push-настройки, безопасное подключение MFA, цифровые слепки браузеров, управление доверенными устройствами, план-график, retry при ошибке расписания | 19 браузерных автотестов |
...
| **Foreman сейчас** | **740-800** | Веб, мобильное приложение и Telegram-бот имеют несколько рабочих слоев защиты: серверные роли, гостевые ограничения, proxy-allowlist, антивирусный контур, контроль опасных форматов, журналы безопасности, осторожный honeypot/bot-defense на входе, HttpOnly cookie-сессию для веба, защиту rate-limit от простой подмены forwarding-заголовков, TOTP/MFA для веб-входа и мобильного админского входа, расширенный цифровой слепок браузера и известные устройства в audit-контуре, наблюдение риск-сигналов входа, управляемые Web Push-оповещения, мобильную APK-сборку с obfuscation, проверку подписи манифеста обновления и SHA-256 APK, регрессии, мегатест и мегатест-2. Диапазон дан консервативно: оценка опирается только на реально внедренные меры, а не на обещания будущего аудита. Foreman уже заметно сильнее обычного внутреннего сайта, но еще не является зрелой промышленной SaaS/ECM-платформой |
...
Мобильное приложение проверяет обновления через настроенный манифест. Перед доверием к `app-update.json` приложение проверяет подпись `app-update.json.sig`; после скачивания APK сверяет SHA-256 файла с тем значением, которое указано в подписанном манифесте обновления. Если манифест, файл APK или контрольная сумма подменены, повреждены или загружены не полностью, установка не должна продолжаться как обычное обновление.
...
Текущая версия в исходниках: `0.2.104+6105`.
