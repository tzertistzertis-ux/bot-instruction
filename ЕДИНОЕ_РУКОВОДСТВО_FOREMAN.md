...
## Веб-браузер, мобильное приложение, Telegram-бот и Web Push

**Версия документа: 13.08.2026, редакция 13**

Версионный PDF-файл для передачи и скачивания: `ЕДИНОЕ_РУКОВОДСТВО_FOREMAN_2026-08-13.pdf`

## Быстро попробовать Foreman

Основной веб-адрес для сотрудников: https://claim-bot-tzertis.amvera.io/foreman/

Диагностика сайта: [https://foreman-api-tzertis.amvera.io/foreman/](https://claim-bot-tzertis.amvera.io/status)
...
## 0.6 Как Foreman читает заявки через распознавание файлов и `office-converter`
...
1) `page` — базовый проход по странице/изображению;
2) `table` — дополнительный проход по табличным зонам, если они имеют признаки таблицы;
3) `handwritten` — дополнительный проход для рукописных, эскизных и шумных зон.
...
|:---|:---|:---|
| `docx` | Foreman читает структуру Word-документа своим parser-ом, без внешнего распознавания, если документ нормально собран | Реально работает в контролируемом прод-режиме; сложная верстка требует ручной сверки |
| `xlsx` | Foreman читает таблицы своим parser-ом | Реализовано и идёт под контролем реальных рабочих шаблонов |
| `doc` | Сначала `office-converter` переводит старый Word-файл в `docx`, затем Foreman читает его как современный документ | Controlled pilot после live smoke converter-а |
| `xls` | Сначала `office-converter` переводит старую Excel-таблицу в `xlsx`, затем Foreman читает ее обычным путем | Controlled pilot после live smoke converter-а |
| `jpg`, `jpeg`, `png` | После проверки безопасности файл идет в Яндекс Vision OCR, затем Foreman parser разбирает результаты | Для фото с объекта используется каскад `page` + возможные `table`/`handwritten` проходы |
| `pdf` | Foreman может использовать текстовый слой PDF или распознавать страницы через Яндекс Vision OCR | Полезно для простых PDF; сложные сканы, чертежи и плохие копии требуют осторожности |

`office-converter` нужен для старых форматов `.doc` и `.xls`. Эти файлы относятся к устаревшему Office-потоку, где возможны зависания и различия в форматах, поэтому конвертер вынесен в отдельный сервис. Он не получает доступ к базе Foreman, не хранит файлы длительно и работает по токену: задача — перевести файл в `docx`/`xlsx`, а дальше все равно разбор выполняет ядро Foreman.
...
| Таблица | Что хранит |
|:---|:---|
| `ocr_extraction_claims` | Сводку по заявке: номер, объект, корпус, секция, источник, метод чтения и уверенность |
| `ocr_extraction_items` | Строки изделий: размеры, формулы, количество, единицы, площадь, цвет, ламинацию и другие признаки |
| `ocr_extraction_quality_issues` | Замечания по строкам: не найдена ширина, высота, формула, количество или найден конфликт |
...
Для админов это не магия и не «черный ящик»: управляемый контур усиления качества. В боевой эксплуатации его включают через feature flags, лимиты, dry-run для рисковых исходящих действий и canary-валидацию на реальных файлах. По умолчанию сырой OCR-текст и сырой ответ провайдера не пишутся в логи без отдельного технического решения.

## 0.7 Как Foreman проверяется перед выпуском в работу

Foreman - не одна кнопка и не один сервер. Это Telegram-бот, сайт, мобильное приложение, `foreman-api`, конвертер документов, PostgreSQL, файлы, письма и правила доступа. Поломка часто возникает не внутри одной детали, а на стыке: сайт отправил новые данные, а сервер понял их по-старому; документ преобразовался, но результат не прикрепился; два человека одновременно изменили одну запись; красивый эскиз остался в коде, но реальный веб-маршрут до него больше не дошел.

Поэтому тесты нужны не ради технического отчета. Они нужны, чтобы сотрудник не оказался первым человеком, который уже после обновления обнаружил пропавший статус, неработающую кнопку, пустой PDF, ошибку письма или потерянный черновик.

### Что именно проверяется

| Вид<br>проверки | Простое объяснение | Какую поломку ловит |
|:---|:---|:---|
| `Smoke` | Быстро проверяет, что система запускается и основные входы открываются | Сервер не собрался, сайт не отвечает, базовый маршрут умер |
| Регрессия | Повторяет уже известные рабочие сценарии после нового изменения | “Вчера работало, сегодня перестало”: вход, журнал, карточки, статусы, PDF, письма |
| Контракты | Проверяет договор между ботом, сайтом, mobile, backend и базой | Одна часть изменила поля или формат ответа, а другая этого не поняла |
| Integration и E2E | Проходит цепочку из нескольких компонентов, почти как пользователь | Ошибку между созданием заявки, обработкой файла, сохранением и получением результата |
| UI и visual | Нажимает реальные кнопки в браузере и сравнивает важный внешний вид с эталоном | Невидимую кнопку, сломанное окно, переполнение экрана, исчезнувший или примитивный рендер |
| Boundary и negative | Проверяет границы и заведомо плохие данные | Слишком большой файл, пустой ввод, неверный адрес, опасный путь, лишнего получателя |
| Security и права | Проверяет, кто что имеет право видеть и менять | Доступ к чужому черновику, запись гостем, утечку секрета, опасное вложение |
| Fault injection | Намеренно имитирует отказ внешней части | Потерю данных при сбое почты, хранилища, OCR или транзакции |
| Concurrency | Запускает два конфликтующих действия одновременно | Двойную отправку заявки, потерянное изменение план-графика, двойную модерацию |
| Coverage | Измеряет, какие строки и развилки действительно исполнили тесты | Новый код, который добавили без достаточной проверки |
| Mutation | Временно вносит маленькую искусственную ошибку и требует, чтобы тесты ее заметили | Красивый, но слабый тест, который проходит даже при неправильном алгоритме |
| Flake rehearsal | Много раз повторяет ранее нестабильный сценарий без автоматического retry | Хрупкий тест, который случайно бывает то зеленым, то красным |

![Многослойный куб проверок Foreman с количеством тестов и процентами покрытия](./assets/foreman-test-layers-cube.svg)

Куб нужно читать снизу вверх только для удобства. Нижние слои дают быструю и широкую проверку выпуска, верхние глубже испытывают рискованные правила и качество самих тестов. Это не рейтинг важности: если убрать любой слой, определенный вид поломки снова останется без своей проверки.

Толщина слоя приблизительно отражает объём тестов. Масштаб специально сжат, иначе регрессионный слой с тысячами запусков занял бы почти весь куб, а небольшие, но важные проверки стали бы невидимыми. Некоторые слои пересекаются и входят сразу в несколько наборов, поэтому числа внутри куба нельзя складывать между собой.

### Два разных шлагбаума: «Мегатест» и «Мегатест-2»

После усиления Foreman использует два самостоятельных тестовых контура. Это не “первая и вторая половина одного теста” и не цепочка “M2 может работать только после M1”.

| Контур | На какой вопрос отвечает |
|:---|:---|
| **Мегатест, или M1** | Не сломало ли изменение обычную работу затронутых компонентов и можно ли рассматривать выпуск этой версии? |
| **Мегатест-2, или M2** | Выдерживают ли особенно рискованные правила более глубокую проверку: искусственную ошибку, конкуренцию, отказ, повторение и реальную временную PostgreSQL? |

Главное правило сформулировано намеренно жестко: **«Мегатест» и «Мегатест-2» - два разных независимых контура.** У каждого своя команда, свой план, свой запуск и свой итоговый отчет. M2 не требует сначала запускать M1 и не читает результат M1. Их можно запускать в любом порядке.

Перед конкретным выпуском необходимость каждого контура определяется отдельно. Для обычного деплоя выполняется M1. Если изменение затрагивает рискованную зону - права, вход, файлы, PostgreSQL, письма, OCR, renderer, план-график, безопасность или внешние интеграции - дополнительно нужен самостоятельный M2. Если по правилам выпуска требуются оба, зеленым должен быть каждый из них по отдельности.

### Как работает усиленный «Мегатест»

Раньше большая проверка могла без разбора тащить за собой всю экосистему. Теперь M1 сначала смотрит, какие файлы и договоры между компонентами изменились, и сам выбирает необходимый объем.

| Что изменили | Что M1 обязан проверить |
|:---|:---|
| Только внутреннюю функцию Telegram-бота | Полный сбор тестов бота, покрытие измененного кода, рендер и PNG-эталоны; сайт, mobile и PostgreSQL без причины не запускаются |
| Foreman Web или его стили | Сборку и покрытие backend, базовый web-smoke, реальные браузерные сценарии и visual regression |
| Общий API, вход или формат заявки | Backend и совместимость всех известных потребителей этого договора |
| SQL, миграцию, транзакцию или блокировку | Backend и специальную локальную PostgreSQL, никогда не рабочую базу Amvera |
| `office-converter` или старые Office-форматы | Unit/negative-тесты конвертера, реальный LibreOffice и связанные проверки backend |
| Связь нескольких компонентов | Сквозной E2E: взять настоящие тестовые документы, обработать их, сохранить результат в отдельной базе и убедиться, что результат доступен |
| Сам механизм тестов или непонятную зону | Полный профиль всей экосистемы, чтобы неизвестность не превратилась в молчаливый пропуск |

Автоматический выбор не является разрешением “на глаз отключить лишний тест”. Каждая пропущенная стадия получает машинное объяснение `NOT_APPLICABLE`: почему она не относится к этому изменению. Если система не может доказать, что компонент не затронут, проверка расширяется, а не сужается.

В M1 входят, когда они относятся к изменению:

- проверка состава релиза, версий инструментов и полноты плана;
- сборка `foreman-api`;
- полный тестовый сбор claim-bot без тихо исчезнувших тестов;
- контроль покрытия обычного и нового кода;
- точные PNG golden-проверки графических эскизов;
- backend regression и отдельный OCR release gate;
- настоящие браузерные сценарии Foreman Web и visual regression;
- тесты `office-converter` с реальным LibreOffice;
- безопасная локальная PostgreSQL;
- сквозной сценарий между документом, converter, API, Web и базой;
- анализ и тесты Foreman Mobile, когда mobile или общий договор действительно затронуты.

Для крупного выпуска есть полный режим `EcosystemRelease`. Он проверяет всю экосистему, даже если отдельный классификатор мог бы выбрать меньше. Самостоятельно понижать выбранный системой объем запрещено.

### Как работает усиленный «Мегатест-2»

M2 не повторяет весь M1 под другим названием. Он берет собственную выбранную зону риска и проверяет то, чего обычной регрессии недостаточно доказать.

| Собственная проверка M2 | Что она доказывает |
|:---|:---|
| Независимый preflight | M2 имеет собственный полный план, видит точные commit-ы и чистые рабочие деревья, не ищет отчет M1 |
| Mutation-проверки | Маленькие искусственные поломки бота, backend и OCR действительно ловятся тестами, а не “выживают” незамеченными |
| Глубокая PostgreSQL | Миграции, rollback, восстановление, блокировки и параллельные действия работают на временной изолированной базе |
| Проверки конкуренции | Одновременная отправка одной заявки, изменение план-графика и модерация не создают двойной результат и не теряют данные |
| Flake rehearsal | Ранее хрупкие backend- и браузерные сценарии повторяются сериями без retry; единичное случайное падение считается проблемой |
| Performance probe | Важное переключение заявки укладывается в установленный бюджет и при этом дает правильный результат |
| Semantic renderer и SVG | Эскиз содержит правильные элементы и размеры, а проверка не выдает условную картинку за доказательство настоящего рендера |
| Контроль инструкций | Действующие инструкции не могут снова связать M1 и M2 или вернуть устаревшую команду запуска |

M2 выбирает риск-пакеты, а не бесконечно запускает все тяжелые проверки. Например, изменение план-графика подключает его browser/flake и mutation-защиту; изменение PostgreSQL - глубокую DB-проверку; изменение OCR - соответствующие mutation-сценарии. Не относящиеся к риску полные Mobile, converter или обычные M1-наборы не повторяются только ради большого счетчика.

### Почему тесты PostgreSQL не трогают рабочую базу

Ни M1, ни M2 не имеют права проводить разрушительные проверки на `foreman-db` в Amvera.

M1 использует заранее предусмотренную локальную тестовую PostgreSQL на компьютере. M2 для глубоких сценариев создает отдельные временные базы и кластеры со случайными именами, проверяет специальную защитную метку и после работы удаляет их. В этих базах можно безопасно столкнуть два одновременных действия, проверить rollback, миграцию и восстановление.

Если адрес базы похож на рабочий, отсутствует тестовая метка или нельзя доказать безопасную принадлежность базы, проверка обязана остановиться. Подмена локальной базы рабочей запрещена даже ради “быстрого подтверждения”.

### Что видит обычный пользователь в Foreman Test Deploy Center

Для запуска не нужно помнить PowerShell-команды. В `ForemanTestDeployCenter.exe` существуют отдельные разделы и отдельные кнопки для M1 и M2:

- обычная проверка изменений запускает M1 с автоматическим выбором компонентов;
- полная проверка экосистемы запускает полный M1;
- самостоятельная риск-проверка запускает M2;
- PostgreSQL-риск M2 выбирается отдельно;
- план, ход выполнения, остановка, упавшая стадия, повтор нужного раздела, журналы и итог показываются в программе;
- настоящий deploy в Amvera не разрешается, пока обязательный для этого выпуска контур не дал корректный зеленый итог.

Программа не склеивает результаты двух контуров. Зеленый M1 не превращает красный M2 в зеленый и наоборот.

### Как принимается результат

Надпись “несколько тестов прошли” еще не разрешает выпуск. У каждого запуска создается собственный машинный отчет. Технический исполнитель обязан убедиться, что:

- итоговый статус равен `OK`;
- отчет действительно разрешает deploy, а не является только диагностикой;
- все обязательные стадии выполнены;
- нет `FAIL`, `SKIP`, `NOT_RUN`, `CANCELLED` или незавершенной стадии;
- проверены именно те commit-ы и файлы, которые собираются отправить;
- после проверки код, настройки, зависимости и эталоны не менялись.

Если после зеленого результата изменился хотя бы один отправляемый файл, старый отчет больше не является разрешением. Нужно создать новый commit и повторить требуемый контур.

### Как понять масштаб проверок в обычных цифрах

Количество тестов и процент покрытия отвечают на разные вопросы.

- **Количество тестов** показывает, сколько отдельных автоматических сценариев было выполнено: вход, заявка, письмо, файл, право доступа, плохие данные и другие ситуации.
- **Покрытие кода** показывает, какую долю программных строк и развилок тесты действительно заставили сработать. Оно не говорит, что система “защищена на такой же процент”.
- **Мутационные проверки** показывают качество самих тестов: заметят ли они специально внесенную ошибку.

По результатам приёмки усиленного контура 10-12 июля 2026 года масштаб выглядит так:

| Что измеряем | Понятный ориентир | Что это означает |
|:---|:---|:---|
| Полный M1 для всей экосистемы | **около 5 000 выполнений автотестов** в 19 технических стадиях | Проверяются бот, backend, Web, OCR, converter, локальная PostgreSQL, сквозная цепочка и Mobile. Обычный выпуск отдельной функции может запускать меньший набор |
| Claim-bot внутри M1 | **около 2 300 тестов** плюс точные проверки PNG-эскизов | Это основная масса команд, заявок, писем, файлов, рендера и старых сценариев бота |
| Backend и OCR внутри M1 | **около 1 500 backend-тестов** и **около 775 OCR-тестов** | Проверяются серверные правила сайта, данные, права, формы, документы и распознавание |
| M2 | **114 искусственных поломок**, **10 циклов временной PostgreSQL** и серии по **20 повторов** ранее хрупких сценариев | M2 измеряется не большим общим счетчиком, а глубиной: тесты должны поймать ошибку, конкуренцию, сбой и нестабильность |

Число `около 5 000` - это понятный масштаб полного M1, а не обещание, что каждый обычный выпуск всегда запускает ровно столько. Если менялся только бот, сайт или converter, M1 автоматически выбирает относящиеся к изменению компоненты. Точное число конкретного выпуска всегда записывается в его свежем отчете.

### Что означает процент покрытия

Представим программу как большую инструкцию из 100 участков.

- Вся накопленная программа - это всё, что было создано за время жизни Foreman: старые команды, новые функции, редкие ошибки и служебные части.
- Новая работа - это функция или исправление, которое только что добавили либо серьёзно переделали для очередного выпуска.

Для руководителя достаточно этой таблицы:

| Компонент | Если всю накопленную программу представить как 100 участков | Как проверяется новая работа |
|:---|:---|:---|
| Claim-bot | тесты автоматически проходят примерно **64 участка из 100** | из каждых 100 участков, которые только что добавили или серьёзно переделали, проверяются примерно **92** |
| Серверная часть сайта `foreman-api` | тесты автоматически проходят примерно **76 участков из 100** | из каждых 100 участков, которые только что добавили или серьёзно переделали, проверяются примерно **98** |
| `office-converter` | тесты автоматически проходят примерно **79 участков из 100** | отдельный обязательный процент для новой работы пока не установлен |
| Foreman Mobile | общий процент пока честно не посчитан | новые Mobile coverage/golden отложены владельцем и не выдаются за готовую работу |

Слово “измененные” в техническом отчете означает именно **то, что недавно добавили или серьёзно переделали**, а не всю программу целиком.

Фразу **“Foreman защищен на 60%” говорить неправильно**. Эти числа показывают не процент безопасности, а то, через какую долю программного кода реально прошли автоматические тесты.

Оставшиеся участки не обязательно являются поломкой или открытой дырой. Среди них есть редкие аварийные ситуации, связь с внешними сервисами и служебный запуск программы. Часть таких мест проверяется другими способами: через браузер, сквозной сценарий, эталонное изображение, имитацию отказа и проверку уже запущенного сервера. Но участок, через который тест не проходил, всё равно считается более рискованным.

Если директору или сотруднику нужно объяснить всё одной фразой, можно сказать так:

**“Перед полным выпуском Foreman выполняет около пяти тысяч автоматических проверок. Они проходят примерно 64 участка из каждых 100 в программе бота и 76 из 100 в серверной части сайта. А всё, что только что добавили или серьёзно переделали, проверяется примерно на 9 участках из 10 или лучше. Самые опасные правила дополнительно испытываются отдельным Мегатестом-2.”**

Для системного администратора точные технические показатели остаются такими:

- claim-bot: около 64% программных выражений, 67% строк и 56% логических развилок; для недавно добавленного или серьёзно переделанного кода - около 92% строк и 89% развилок;
- `foreman-api`: около 75% выражений, 76% строк, 63% развилок и 81% функций; для недавно добавленного или серьёзно переделанного кода - около 98% строк и 86% развилок;
- `office-converter`: около 79% строк, 73% развилок и 69% функций.

### Что сейчас защищено особенно тщательно

| Рабочая зона | Что проверяется |
|:---|:---|
| Вход, роли и права | Неверный вход не роняет сервер, гость не получает запись, пользователь не управляет чужим черновиком, отключенные сессии перестают работать |
| Заявки и журнал | Номера, обязательные поля, черновики, отправка, порядок журнала, старые и новые форматы |
| Рендер, PDF и e-mail | Настоящий HTTP-маршрут доходит до графического рендера, четыре типа заявок формируют документы и проходят безопасную тестовую отправку письма |
| План-график и таймлайн | Даты, перенос карточек, статусы, итоги, PDF, период и одновременное изменение |
| Файлы и документохранилище | Типы, размеры, антивирусный вердикт, безопасный путь, права скачивания, rollback и отказ внешнего сервиса |
| OCR и converter | DOC/DOCX обрабатываются своим маршрутом, JPG/PNG/PDF - предусмотренным OCR-маршрутом, старые Office-форматы проходят через converter; проверяются конфликтующие и неполные данные |
| PostgreSQL | Миграции, транзакции, rollback, конкурентные действия и сквозное сохранение проверяются только локально и изолированно |
| Веб-интерфейс | Реальные кнопки и окна в браузере, мобильная ширина веб-страницы, visual regression и ранее нестабильный план-график |
| Telegram-бот | Команды, заявки, письма, вложения, golden-рендер, временные сетевые отказы и защита важных правил мутациями |
| Foreman Mobile | Существующие тесты и анализ сохраняются; новые coverage/golden для Flutter отложены владельцем и не выдаются за выполненную работу |

### Что намеренно не делается автоматически на рабочей системе

Тестовый контур не должен сам создавать проблему, от которой защищает. Поэтому перед обычным деплоем он не рассылает массовые письма реальным сотрудникам, не меняет живую Google-таблицу, не атакует рабочий сайт тяжелым DDoS, не выполняет разрушительные SQL-команды в Amvera и не устанавливает APK на чужие телефоны.

После deploy выполняются отдельные осторожные smoke-проверки уже живой Amvera: доступность, разрешенные маршруты, запреты гостя и безопасное чтение. Они подтверждают, что новая версия поднялась на сервере, но не заменяют M1 или M2.

### Почему одного ручного клика недостаточно

Человек обычно проверяет один счастливый путь: открыл сайт, вошел, увидел журнал. Автоматические контуры дополнительно пробуют плохой пароль, пустые и старые данные, чужие права, граничный размер файла, два действия одновременно, отказ почты, ошибку хранилища, повторный запуск и внешний вид на узком экране.

Ручная проверка остается полезной для здравого смысла и оценки удобства. Но она не может заменить систематический повтор всех опасных вариантов.

### Честное ограничение

Даже усиленные M1 и M2 не дают математической гарантии “регрессий больше не будет”. Они значительно уменьшают риск и превращают уже найденные поломки в постоянные автоматические проверки. Но тесты не заменяют резервные копии, контроль диска и базы, исправные доступы к почте и внешним сервисам, post-deploy smoke и внимательную эксплуатацию.

Практическое правило Foreman такое: важная поломка не должна просто исправляться один раз. Для нее создается надежная проверка, которая входит в подходящий независимый контур и мешает той же ошибке незаметно вернуться через несколько месяцев.
...
Общий гостевой логин не означает одну общую сессию. Каждый вошедший гость
получает свою серверную сессию посетителя. Поэтому если один подрядчик вошел и
не нажал `Выйти`, другой гость не должен быть выбит из системы только из-за
этого. По умолчанию гостевой логин может держать до 100 активных сессий, а
обычные пользователи - до 3.
...
| HttpOnly cookie-сессия в веб-браузере | Защита веб-входа от кражи токена обычным JavaScript | Foreman Web остается в сессии после входа, но страница не читает основной секрет авторизации из `localStorage` |
...
| Нормализация IP на сервере | Rate-limit, bot-defense и журналы безопасности | Подставленные `X-Forwarded-For` и похожие заголовки не считаются доверенным IP для блокировок |
...
- широкий proxy “любой `/api/*`” закрыт, оставлены только проверенные маршруты;
...
- безопасность закреплена обычными тестами, двумя независимо выбираемыми контурами M1/M2 и live-smoke после деплоя; отдельный браузерный тест проверяет, что случайный клик `Подключить MFA` не создает QR-код и не пугает сотрудника неожиданной настройкой;
...
1. В `Мои реквизиты` сотрудник нажимает `Подключить MFA`.
2. Foreman сначала показывает предупреждение: на телефоне уже должно быть установлено приложение-аутентификатор.
3. Если сотрудник нажимает `Отмена`, ничего страшного не происходит: вход по логину и паролю не меняется.
4. QR-код создается только после осознанного подтверждения `Приложение установлено, продолжить`.
5. MFA считается подключенной только после правильного 6-значного кода.
6. После подключения нужно сохранить резервные коды. Они показываются один раз и нужны, если телефон потерян или приложение удалено.
...
1. Нажать на свое ФИО в верхней панели.
2. Открыть `Мои реквизиты`.
3. В блоке `Push-уведомления` нажать `Включить push`.
4. Разрешить уведомления в браузере, если браузер спросит.
...
1. Установить на смартфон Яндекс Ключ, Google Authenticator, Microsoft Authenticator, Aegis, 2FAS или другое приложение для одноразовых кодов.
2. В Foreman Web нажать свое ФИО в верхней панели.
3. Открыть `Мои реквизиты`.
4. В блоке `Двухфакторная защита входа` нажать `Подключить MFA`.
5. Прочитать предупреждение. Если приложения на телефоне еще нет, нажать `Отмена`.
6. Если приложение установлено, нажать `Приложение установлено, продолжить`.
7. Отсканировать QR-код телефоном.
8. Ввести 6 цифр из приложения.
9. Сохранить резервные коды в надежном месте.
...
| `/rename_mail_claim <ошибочный номер> <правильный номер>` | Админ | Навсегда переименовывает ошибочно созданную почтовую заявку во всех связанных почтовых реестрах; старый номер не должен восстановиться при следующем разборе почты |
...
| `еженедельник`, `/damage_forecast`, `/weekly_control` | Админ | Живой прогноз на ближайшие 72 часа: где скоро истечет контрольный срок, кому бот может написать и где может потребоваться руководство |
...
- если бот уже создал ошибочную почтовую заявку, администратор навсегда переименовывает ее командой `/rename_mail_claim <ошибочный номер> <правильный номер>`;
...
Лариса подключается только после корректного ответа Оксаны с номером запуска вида `С-26-00677`. В первые 24 часа писем Ларисе быть не должно.
...
### Постоянное переименование ошибочной почтовой заявки

Команда:

`/rename_mail_claim <ошибочный номер> <правильный номер>`

Пример для ошибочно распознанного имени скана:

`/rename_mail_claim REENSHOT-20260811 ПР-09`

Команда доступна только администратору. Она нужна, когда бот уже создал в журнале и таймлайне ложный номер из имени файла, темы письма или OCR. Команда переносит связанную почтовую историю и контроль на правильный номер, сохраняет запись о ручном исправлении в PostgreSQL и ставит постоянную связь «старый номер → правильный номер».

После этого при повторном разборе того же письма, восстановлении индекса или перезапуске бота ложный номер не должен появиться снова. Команда не удаляет вложение и не меняет документ: она исправляет только номер заявки в почтовых реестрах. Если ошибка относится лишь к одной временной ожидающей записи, а не к ошибочно созданной заявке, используйте `/clear_mail_pending <номер>`.
