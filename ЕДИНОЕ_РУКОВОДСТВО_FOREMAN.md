...
**Версия документа: 14.06.2026**

Версионный PDF-файл для передачи и скачивания: `ЕДИНОЕ_РУКОВОДСТВО_FOREMAN_2026-06-14.pdf`
...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота, серверная защита гостевого наблюдателя, базовая проверка proxy-allowlist | около 910+ автотестов в текущем составе smoke/regression-контуров |
...
| Тесты мобильного приложения | Экраны, маршруты, кэш заявок, заявки на заполнения/расходники, защищенное хранение офлайн-черновиков и очереди отправки, проверка безопасного обновления APK, мобильный MFA-вход и совместимость с backend | 82 автотеста |

Итого в обычном мега-тесте проходит примерно 1120+ автопроверок, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются важные сценарии, тестов становится больше. Самым надежным источником точного числа всегда остается свежий вывод `pre_deploy_all.ps1`, `release_check.ps1`, `npm test` и `flutter test`.

Внутри регрессионного ядра проверки распределяются так:

| Подстадия<br>regression core | Количество |
|:---|:---|
| Jest-тесты `foreman-api` | 608 |
...
Оценка тестового контура, например `865 / 1000`, - это внутренняя риск-шкала Foreman. Она показывает зрелость автоматических проверок и их способность защищать рабочую систему от регрессий.
...
Число `865 / 1000` означает высокий уровень зрелости тестового контура Foreman для текущего масштаба проекта. Локальные автотесты закрывают рабочую логику и основные регрессии, а внешние зависимости - Amvera, Gmail/SMTP, Telegram API, Google-доступы и установка APK на настоящий Android - проверяются отдельным live-smoke после деплоя.
...
| **Foreman сейчас** | **730-785** | Веб, мобильное приложение и Telegram-бот имеют несколько рабочих слоев защиты: серверные роли, гостевые ограничения, proxy-allowlist, антивирусный контур, контроль опасных форматов, журналы безопасности, осторожный honeypot/bot-defense на входе, HttpOnly cookie-сессию для веба, защиту rate-limit от простой подмены forwarding-заголовков, TOTP/MFA для веб-входа и мобильного админского входа, расширенный цифровой слепок браузера и известные устройства в audit-контуре, наблюдение риск-сигналов входа, управляемые Web Push-оповещения, регрессии, мегатест и мегатест-2. Диапазон дан консервативно: оценка опирается только на реально внедренные меры, а не на обещания будущего аудита. Foreman уже заметно сильнее обычного внутреннего сайта, но еще не является зрелой промышленной SaaS/ECM-платформой |
...
| Foreman Mobile | 735-790 | Актуальная APK-сборка в production работает только с утвержденным сервером Foreman, запрещает cleartext HTTP, очищает неподходящий сохраненный API-адрес, не использует дефолтный сервисный PIN, выпускается с постоянной production-подписью, нормальным applicationId `ru.tzertis.foreman`, отключенным backup и проверкой SHA-256 у скачиваемого APK. Офлайн-черновики и очередь отправки хранятся в защищенном хранилище с миграцией старых данных из `SharedPreferences`, лишнее Android-разрешение `READ_MEDIA_AUDIO` удалено из release-сборки, мобильный админский вход поддерживает MFA challenge, а мобильный API усилен: уязвимая Excel-библиотека `xlsx` убрана из backend, импорт словарей ограничен по размеру/листам/строкам, `document-storage` закрыт стандартным `AuthGuard` | Провести независимый мобильный пентест, добавить отдельные контрольные проверки ручной публикации APK на Amvera, регулярно проверять зависимости и протестировать установку/обновление на нескольких реальных Android-устройствах |
| Система Foreman в целом | 730-785 | Защита не держится на одной кнопке: права проверяются сервером, файлы проходят антивирусный контур, лишние proxy-маршруты режутся до backend, веб-вход переведен на HttpOnly cookie, вход дополнительно прикрыт honeypot/bot-defense и TOTP/MFA, мобильный админский вход также проходит через MFA challenge, rate-limit не доверяет подставленным forwarding-заголовкам, расширенный цифровой слепок браузера, известные устройства и риск входа пишутся в audit, важные события могут дублироваться управляемым Web Push, изменения проверяются тестовыми контурами | Внешний пентест, WAF/anti-DDoS, SSO, включение risk-based step-up после накопления наблюдений, формальная карта реагирования на инциденты, регулярная проверка зависимостей и секретов |
...
- Foreman Mobile хранит офлайн-черновики и очередь отправки в защищенном хранилище, автоматически переносит туда старые локальные данные из `SharedPreferences`, а release-сборка не запрашивает лишнее разрешение `READ_MEDIA_AUDIO`;
...
Текущая версия в исходниках: `0.2.103+6104`.
