...
Для старых писем и старых внешних вложений предусмотрена отдельная ретро-проверка. Это не происходит от клика сотрудника по карточке, чтобы не тормозить веб-браузер и не гонять сотни файлов во внешний сервис внезапно. Администратор запускает ее отдельной командой партиями: сначала сухой план, потом реальную проверку. После этого старые файлы тоже получают запись в журнале и понятную отметку в веб-карточке.
