...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 1959 автопроверок, плюс 2 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
Backend mutation pilot summary: killed=5 survived=0 total=5
```
...
Отдельно добавлен backend mutation pilot Foreman Web: 5 точечных мутантов по Kaspersky и документохранилищу. Он проверяет, что backend не принимает `no_verdict` за чистый файл, не игнорирует HTTP-ошибки Kaspersky, не пропускает серую зону без полного анализа и не отключает серверные запреты для рискованных файлов документохранилища.
