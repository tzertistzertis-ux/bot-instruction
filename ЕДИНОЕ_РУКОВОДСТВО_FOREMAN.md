...
Даже если на сервере осталась старая переменная со списком форматов для антивирусной отправки, Foreman сохраняет базовый рабочий набор: `pdf`, `doc`, `docx`, `xls`, `xlsx`, `rtf`, `jpg`, `jpeg`, `png`, `dwg`, `dxf`. Это защищает от ситуации, когда документ разрешен в интерфейсе, но случайно режется внутренним антивирусным списком.

При этом Foreman не верит одному названию файла. Если обычный текстовый файл переименовать в `.dwg` или `.dxf`, система должна увидеть несоответствие и не принять такой файл как чертеж. Настоящий DWG/DXF проходит дальше, а переименованная подделка блокируется до отправки в Kaspersky.
...
| Backend-тесты антивируса и документохранилища Foreman | Политика `clean/no_verdict/api_unavailable`, полная отправка файла в Kaspersky, сохранение обычных файлов, `rtf` и строительных `dwg/dxf` с риск-меткой при неполном ответе Kaspersky, защита от старой серверной переменной форматов, запрет переименованного текста под видом `dwg/dxf`, серверный запрет скачивания без принятия риска, запрет прикрепления и e-mail-отправки рискованных файлов, запрет HTML Smuggling-файлов и маскировки HTML/JavaScript внутри `txt/doc` | 45 автотестов |
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 1993 автопроверки, плюс 3 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
