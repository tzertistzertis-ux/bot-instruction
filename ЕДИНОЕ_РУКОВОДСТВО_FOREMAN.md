...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота, серверная защита гостевого наблюдателя, базовая проверка proxy-allowlist | около 780+ автотестов в текущем составе smoke/regression-контуров |
...
Итого в обычном мега-тесте проходит примерно 950+ автопроверок, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются важные сценарии, тестов становится больше. Самым надежным источником точного числа всегда остается свежий вывод `pre_deploy_all.ps1`, `release_check.ps1`, `npm test` и `flutter test`.

Внутри регрессионного ядра проверки распределяются так:

| Подстадия<br>regression core | Количество |
|:---|:---|
| Jest-тесты `foreman-api` | 486 |
...
| Backend-тесты bot-defense/honeypot | Скрытое поле входа не видно человеку, один подозрительный сигнал только журналируется, временная блокировка включается осторожно и только при повторных/сочетанных признаках, админские настройки и снятие блокировки недоступны обычному пользователю | 14 целевых автотестов в составе backend-проверок |
...
| Proxy allowlist Foreman через `claim-bot` | Широкий прокси "любой `/api/*`" закрыт; проходят только доказанные рабочие маршруты, неизвестные backend-маршруты режутся на границе `claim-bot` с `404 foreman_proxy_route_not_allowed` | 5 точечных unit-тестов плюс отдельный live-smoke после деплоя |
...
| Honeypot/bot-defense на входе | Один сигнал = журнал и риск, повторные/сочетанные признаки = возможная временная блокировка | Система отличает осторожную защиту от грубого “банить всех подряд” |
...
Дополнительно у Foreman включен `honeypot` на форме входа. Это тихая ловушка для автоматических программ: живой пользователь ее не видит и не заполняет, а простой бот может “клюнуть” на скрытое поле или отправить форму слишком механически. Такой сигнал попадает в журнал bot-defense.

Важно: один honeypot-сигнал сам по себе не блокирует сотрудника. Это сделано специально, чтобы не наказать живого человека из-за странного браузера, автозаполнения или сетевой задержки. Временная блокировка применяется осторожно: только когда признаки повторяются или складываются с другими тревожными факторами, например быстрыми попытками входа и большим числом неверных паролей. Администратор видит журнал, может снять активную блокировку и при необходимости перевести защиту в режим “только журнал”. Honeypot дополняет rate-limit, но не заменяет MFA, WAF/anti-DDoS и внешний security-мониторинг.
...
| Proxy allowlist `claim-bot` -> Foreman API | Через публичный `claim-bot` проходят только заранее разрешенные рабочие API-маршруты; неизвестные или лишние backend-маршруты блокируются до обращения к `foreman-api` |
...
| Honeypot/bot-defense | Защита формы входа от простых автоматических атак | Скрытые и поведенческие сигналы попадают в журнал; блокировка включается только осторожно, при повторных или сочетанных признаках |
...
| **Foreman сейчас** | **690-750** | Веб, мобильное приложение и Telegram-бот имеют несколько рабочих слоев защиты: серверные роли, гостевые ограничения, proxy-allowlist, антивирусный контур, контроль опасных форматов, журналы безопасности, осторожный honeypot/bot-defense на входе, управляемые Web Push-оповещения, регрессии, мегатест и мегатест-2. Диапазон повышен умеренно: honeypot усилил защиту входа, но не заменил независимый пентест, WAF/anti-DDoS, SSO/MFA, формальный SOC/мониторинг и регулярный внешний security-аудит |
...
|:---|:---|:---|:---|
| Foreman Web в браузере | 700-760 | Авторизация, серверные роли, защита гостя, лимиты скачивания, журнал безопасности, proxy-allowlist, honeypot/bot-defense на входе, антивирусный контур, запрет опасных вложений, профильные Web Push-подписки, админские правила push-событий, UI/visual-регрессии | Ужесточить CSP без `unsafe-inline`, добавить внешний WAF/anti-DDoS, SSO/MFA и независимый пентест |
| Telegram-бот и claim-bot | 700-760 | Проверка ролей, безопасные команды, quarantine/risk-gate файлов, ALARM-письма при вирусах, push по важным почтовым событиям, proxy-allowlist, диагностика, тесты полного regression-контура | Добавить более формальный мониторинг алертов, регулярный внешний аудит и отдельную карту реагирования на инциденты |
| Foreman Mobile | 620-700 | Актуальная APK-сборка в production работает только с утвержденным сервером Foreman, запрещает cleartext HTTP, очищает неподходящий сохраненный API-адрес и не использует дефолтный сервисный PIN | Перейти с debug-signing и `com.example...` на постоянную production-подпись и нормальный applicationId, затем провести мобильный security-review |
| Система Foreman в целом | 690-750 | Защита не держится на одной кнопке: права проверяются сервером, файлы проходят антивирусный контур, лишние proxy-маршруты режутся до backend, вход дополнительно прикрыт honeypot/bot-defense, важные события могут дублироваться управляемым Web Push, изменения проверяются тестовыми контурами | Внешний пентест, WAF/anti-DDoS, SSO/MFA, формальная карта реагирования на инциденты, регулярная проверка зависимостей и секретов |
...
- honeypot/bot-defense добавлен как осторожная защита входа от простых ботов: он журналирует подозрительные сигналы и может временно ограничить доступ только при повторных или сочетанных признаках;
- мобильная APK-сборка исключает увод пользователя на чужой сервер в production;
...
В Foreman Web добавлена дополнительная защита входа `honeypot/bot-defense`. Ее задача - не мешать сотруднику, а тихо замечать простые автоматические попытки входа.

Работает это так: на форме входа есть скрытые технические признаки, которые обычный человек не видит и не должен заполнять. Если программа пытается отправить форму “как робот”, слишком быстро или с заполнением скрытого поля, Foreman записывает это в журнал bot-defense. Для администратора это выглядит как понятный журнал подозрительных сигналов, а не как загадочная ошибка.

Главное правило: один сигнал не блокирует человека. Foreman не должен превращаться в систему, которая из-за одного странного клика не пускает сотрудника на работу. Временная блокировка возможна только осторожно - при повторении подозрительных признаков или при сочетании с другими факторами, например большим числом неверных паролей.
