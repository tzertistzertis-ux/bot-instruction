...
Что изменено в редакции 7: в руководство добавлен новый контур Foreman OCR и отдельный `office-converter` для старых Word/Excel-файлов. Обновлены тестовые числа после усиления OCR: основной мега-тест теперь включает OCR release gate, а мегатест-2 проверяет OCR mutation inventory и при необходимости запускает полный OCR mutation pilot. Формулировки оставлены честными: OCR уже помогает читать заявки и замечать пропуски, но не отменяет ручную сверку сложных документов и включается в production только через controlled pilot.
...
## 0.6 Как Foreman читает заявки через OCR и office-converter

Foreman постепенно перестает быть только местом, куда складывают файлы. Теперь он умеет аккуратно читать часть входящих заявок: достать номер, объект, корпус, секцию, строки изделий, размеры, формулы стеклопакетов, количество и признаки проблемной строки. Это не магия и не замена человеку. Это внимательный помощник, который смотрит в документ раньше офиса и говорит: "здесь, кажется, не хватает высоты", "здесь не видно формулы", "здесь количество написано странно".

Главное правило осталось прежним: OCR запускается только после защитного контура файлов. Если файл не получил чистый вердикт безопасности, Foreman не должен спокойно отдавать его в распознавание как обычный рабочий документ. Сначала безопасность, потом чтение.

Что умеет OCR-контур сейчас:

| Источник | Как обрабатывается | Текущий статус |
|:---|:---|:---|
| `docx` | Читается нативным parser-ом без внешнего OCR, если структура документа нормальная | Готово для controlled production, но странная верстка требует ручной сверки |
| `xlsx` | Читается нативным parser-ом таблиц | Поддержано, но нужен аккуратный rollout на реальных рабочих шаблонах |
| `doc` | Сначала переводится отдельным `office-converter` в `docx`, затем идет по native-пути | Подходит для controlled pilot после live smoke converter-а |
| `xls` | Сначала переводится отдельным `office-converter` в `xlsx`, затем идет по native-пути | Подходит для controlled pilot после live smoke converter-а |
| `jpg`, `jpeg`, `png` | Может идти через внешний OCR-провайдер, если включены production-флаги и лимиты | Только canary/controlled режим, потому что фото с объекта бывают шумными |
| `pdf` | Может использовать текстовый слой и/или OCR по страницам | Полезно для простых PDF, но сложные чертежи и сканы требуют осторожности |

`office-converter` вынесен в отдельный сервис не случайно. Старые `.doc` и `.xls` живут в мире LibreOffice, конвертации и возможных подвисаний. Поэтому converter не получает доступ к базе Foreman, не хранит файлы постоянно и работает по токену. Его задача узкая: принять старый Office-файл, вернуть современный `.docx` или `.xlsx`, а дальше Foreman уже разбирает документ обычным путем. Если converter выключен или недоступен, backend не должен падать: файл остается для ручной обработки или для более позднего безопасного повтора.

Результат распознавания хранится в PostgreSQL. Полная structured-картина остается в `request_content_extractions.canonical_json`, а первый аналитический слой уже разложен в таблицы:

| Таблица | Что хранит |
|:---|:---|
| `ocr_extraction_claims` | Сводку по заявке: номер, объект, корпус, секция, источник, метод, уверенность |
...
Это уже не просто "текст из картинки". Но это еще и не финальная витрина аналитики. Для полноценной production-аналитики остаются рабочие задачи: backfill старых extraction-ов, расширение item-модели под реальные отчеты, отдельный report/UI для руководителя и еще более плотные row-level quality tests на неоднозначные размеры, мусорные формулы и странные количества.

Как это помогает прорабу и офису:

- если заявка пришла с пропущенной высотой, количеством или формулой, Foreman может добавить понятный OCR-блок в подтверждающее письмо;
- если в одном письме по одной заявке пришли два файла, система умеет объединять дополняющие данные, а не считать каждый файл отдельной истиной;
- если имя файла или тема письма врут, а внутри документа найден нормальный номер заявки, главным считается внутренний canonical номер;
- если данные конфликтуют, Foreman должен предупредить, а не тихо выбрать случайную версию.

Подтверждающие письма прорабам устроены бережно: OCR-замечания добавляются отдельным блоком и группируются по заявкам. Заявка без проблем не попадает в красный блок "за компанию". Технические поля вроде `items[0].factoryMarking` не должны уходить человеку; в письме пишется нормальный рабочий язык: "заводская маркировка", "ширина", "высота", "формула стеклопакета", "количество".

Администратору важно помнить: OCR - это усиление контроля, а не автоматический судья. В production его правильно включать через feature flags, малые лимиты, dry-run для рискованных outbound-действий и canary-набор реальных файлов. Сырой OCR-текст и сырой ответ провайдера по умолчанию не должны храниться без отдельной осознанной причины: в рабочих документах может быть лишняя информация, которой не место в логах.
