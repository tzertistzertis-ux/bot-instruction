...
**Версия документа: 14.06.2026, редакция 3**

Версионный PDF-файл для передачи и скачивания: `ЕДИНОЕ_РУКОВОДСТВО_FOREMAN_2026-06-14.pdf`

Что изменено в редакции 3: для Foreman Mobile зафиксированы версия мобильного релиза `0.2.105+6106`, управление доверенными мобильными устройствами, привязка мобильных сессий к экземпляру Foreman Mobile на конкретном смартфоне, возможность заблокировать доступ к мобильному приложению на потерянном смартфоне сотрудника, без блокировки остальных устройств, APK obfuscation, подписанный манифест обновления `app-update.json.sig`, проверка подписи манифеста перед SHA-256 скачанного APK, а также обновлены честные оценки безопасности и количество актуальных тестов.
...
| Тесты мобильного приложения | Экраны, маршруты, кэш заявок, заявки на заполнения/расходники, защищенное хранение офлайн-черновиков и очереди отправки, проверка безопасного обновления APK, подпись манифеста обновления, мобильный MFA-вход, постоянный `installationId`, отправка безопасного `deviceProfile` и совместимость с backend | 90 автотестов |

Итого в обычном мега-тесте проходит примерно 1135+ автопроверок, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются важные сценарии, тестов становится больше. Самым надежным источником точного числа всегда остается свежий вывод `pre_deploy_all.ps1`, `release_check.ps1`, `npm test` и `flutter test`.
...
| **Foreman сейчас** | **745-805** | Веб, мобильное приложение и Telegram-бот имеют несколько рабочих слоев защиты: серверные роли, гостевые ограничения, proxy-allowlist, антивирусный контур, контроль опасных форматов, журналы безопасности, осторожный honeypot/bot-defense на входе, HttpOnly cookie-сессию для веба, защиту rate-limit от простой подмены forwarding-заголовков, TOTP/MFA для веб-входа и мобильного админского входа, расширенный цифровой слепок браузера, доверенные устройства и наблюдение риск-сигналов входа в audit-контуре, управляемые Web Push-оповещения, мобильную APK-сборку с obfuscation, проверку подписи манифеста обновления и SHA-256 APK, управление доверенными мобильными устройствами с отзывом сессий экземпляра Foreman Mobile на конкретном смартфоне, регрессии, мегатест и мегатест-2. Диапазон дан консервативно: оценка опирается только на реально внедренные меры, а не на обещания будущего аудита. Foreman уже заметно сильнее обычного внутреннего сайта, но еще не является зрелой промышленной SaaS/ECM-платформой |
...
Текущая версия в исходниках: `0.2.105+6106`.
