...
Foreman работает с обычными рабочими файлами: заявки Word и Excel, PDF, фотографии с объекта, вложения из писем и документы в документохранилище. В этих файлах обычно нет коммерческой тайны уровня банка, но они все равно приходят из разных компьютеров и телефонов. Поэтому сейчас у Foreman есть отдельный антивирусный контур.

Простыми словами: файл сначала проверяется, и только потом попадает в обычную работу. Если файл выглядит опасным или проверка дала красный результат, система не должна спокойно положить его рядом с рабочими документами.
...
| Внешняя антивирусная проверка | Отправляет хеш файла, а при необходимости сам файл во внешний антивирусный API | Помогает поймать известные угрозы без установки тяжелого антивируса на тариф Amvera |
...
Если внешний сервис говорит, что файл опасный или подозрительный, файл блокируется. Если сервис временно не дал окончательный ответ, система фиксирует это в журнале отдельным статусом: такой случай не скрывается “под ковром”.
...
|---|---|---|
| `Smoke-тесты` | Это быстрая проверка “дышит ли система”: сайт открывается, сервер отвечает, основные двери не заклинило | Грубую поломку запуска: приложение не собралось, сайт не открывается, базовый маршрут умер |
| `Регрессионные тесты` | Это защита старой логики при новых обновлениях. Если добавили новый раздел, старые заявки, журнал, вход, план-график и письма должны работать по-прежнему | Случаи “вчера работало, сегодня перестало”, случайную смену правил, сортировок, прав, статусов и форматов данных |
| `Contract-тесты` | Проверка “договора” между частями системы: какие поля есть у заявки, карточки, статуса, вложения, ответа API | Поломку связки веба, мобильного приложения, Telegram-бота, backend и базы данных |
| `Integration/e2e-тесты` | Integration-тесты проверяют несколько частей вместе. E2E - это End-to-End, сквозная проверка от начала до конца, почти как действия живого пользователя | Ошибки на стыке модулей: backend, маршруты, заявки, план-график, письма, файлы |
| `UI-тесты` | Проверка реального поведения в браузере: кнопки, карточки, статусы, перетаскивание, мобильный размер экрана | Ошибки, которые видны только пользователю |
| `Boundary-тесты` | Проверка границ: ровно столько, сколько можно, и сразу чуть больше, чем можно | Слишком большие файлы и запросы, превышение лимита получателей письма, опасные границы путей к файлам |
| `Negative-тесты` | Проверка плохих данных: система получает неправильный файл, неверный адрес, пустой ввод или запрещенный путь и должна спокойно отказать | Падение сервера вместо понятного отказа, обход правил, прием опасных вложений или мусорных данных |
| `Security/audit-тесты` | Проверка защитного контура файлов: файл должен пройти проверку, а журнал должен записать итог без содержимого файла, паролей и токенов | Случаи, когда опасный файл попадает в обычное хранилище или диагностика раскрывает лишнее |
| `Fault injection-тесты` | Проверка отказов: искусственно создается неприятность, например письмо не отправилось или хранилище не сохранило файл | Потерю данных при сбое, старый токен отключенного пользователя, повтор recovery-кода, отсутствие rollback при ошибке |
| `Visual regression-тесты` | Проверка внешнего вида: важный экран сравнивается с эталоном, чтобы после правки интерфейс не “поехал” | Съехавшие карточки, кнопки, ошибки загрузки, план-график и мобильный вид |
| `Мутационное тестирование` | Это проверка не программы, а качества самих тестов. В код временно вносят маленькую искусственную ошибку и смотрят: тесты действительно ее поймали или просто “прошли для галочки” | Слабые проверки, слишком общие ожидания, тесты без точного результата |

### Зачем нужны регрессионные тесты

Регрессионные тесты - один из самых важных слоев. Они отвечают за спокойствие пользователей после обновления.

Пример: добавили новую возможность в план-график или изменили правила номера заявки. Регрессионные тесты проверяют, что при этом не изменилось то, что уже было принято и работало:

- вход в кабинет;
- журнал заявок;
- создание и открытие карточки;
- права пользователя;
- сортировка и фильтры;
- таймлайн;
- план-график;
- PDF и тестовая отправка письма;
- мобильная совместимость.

То есть пользователь должен быть уверен: программа развивается, но ее прежняя логика не “поплыла” без предупреждения.

### Зачем нужны smoke-тесты

Smoke-тесты не доказывают, что вся программа идеальна. Их задача проще и грубее: быстро понять, что после изменения система вообще запускается и основные части живы.

Это как первая проверка после ремонта электрики: включился ли свет, не выбило ли автомат, открывается ли главный щиток. Такая проверка не заменяет полный осмотр, но без нее нельзя двигаться дальше.

В Foreman smoke-тесты помогают быстро поймать:

- приложение не собирается;
- сайт не открывается;
- health/readiness не отвечает;
- базовый сценарий заявки умер;
- мобильная часть перестала проходить минимальный рабочий сценарий.

### Зачем нужны мутационные тесты

Обычный тест может быть красивым, длинным и при этом слабым. Например, он открыл страницу, но не проверил главный результат. Или создал заявку, но не убедился, что номер нормализовался правильно.

Мутационное тестирование работает жестче: в код временно вносится маленькая ошибка. Например, меняется условие, граница, формат номера или правило статуса. Если тесты это не заметили, значит в проверке есть дырка.

Так тесты превращаются не в формальность, а в фильтр качества. Они начинают проверять не просто “код выполнился”, а “система выдала именно тот результат, который задумывался”.

Важно: мутационные проверки не считаются как отдельные “еще 100 тестов” в обычном мега-тесте. Это другой вид работы. Сначала специально портится маленькое правило, потом смотрят, какие обычные тесты это поймали, а какие нет. Если тест не поймал важную ошибку, в него добавляется точная проверка результата.

После такой ревизии усиленные проверки уже становятся частью обычного набора тестов и входят в мега-тест. Поэтому мутационное тестирование в Foreman работает как аудит качества тестов, а не как отдельная кнопка “запустить еще один большой набор”.

Примеры зон, где мутационная ревизия уже усиливала тесты:

| Зона | Что проверяли мутациями | Что закреплено обычными тестами |
|---|---|---|
| Номера заявок | Границы и формат: `ЖА-1`, `ЖА-9999`, запрет `ЖА-0`, `ЖА-10000`, коротких/длинных префиксов, нормализация `жа123` в `ЖА-123` | Номер должен сохраняться и нормализоваться именно по рабочему правилу, а мусор должен отсеиваться |
| Auth и вход | Неверный логин не должен давать 500, защита от частого подбора пароля должна срабатывать мягко | Ошибки входа возвращают контролируемый отказ, а не падение сервера |
| Журнал заявок | Порядок и видимость заявок не должны случайно поменяться после правок | Новые и незаведенные заявки не должны “уплывать” вниз из-за ошибочной сортировки |
| План-график | Даты, границы периода, перенос карточек, итоги и статус `Изготовлено` | Карточки остаются в нужных датах, итоги пересчитываются, статус сохраняется |
| Контроль заявок ботом | Эскалация по незаведенным заявкам и письма ответственным | Бот не просто пишет письмо, а соблюдает ожидаемый сценарий контроля |
| Вложения | Граничный размер файла и опасные типы вложений | Файл на допустимой границе проходит, опасный или слишком большой файл блокируется |
...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота | 358 автотестов |
...
Итого в обычном мега-тесте сейчас проходит примерно 521 автопроверка, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются новые важные сценарии, тестов становится больше.

Внутри регрессионного ядра эти 358 проверок распределяются так:

| Подстадия regression core | Количество |
|---|---:|
| Unit-тесты `foreman-api` | 272 |
| E2E-тесты `foreman-api` | 14 |
| Planning smoke | 28 |
| Real-flow smoke | 1 |
| Request-types smoke | 4 |
| Foreman Mobile compatibility smoke | 3 |
| Claim-bot release check smoke | 36 |
...
| Полная регрессия Telegram-бота (`full`) | Команды, рендер, golden-проверки, старые сценарии, защита от регрессий, отдельная pytest-матрица реестра запусков, журнал антивирусных проверок и просмотр заблокированных файлов | 1821 автотест плюс render/golden-проверки |
...
| Дополнительные backend regression-скрипты | Хранение документов, обновление данных, input/render-логика | 2 отдельных regression-скрипта |

Итого в последнем успешном прогоне `мегатеста-2` прошло примерно 1855 автопроверок, плюс 2 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
```text
Mutation pilot summary: killed=18 survived=0 total=18
```

Это означает: все выбранные искусственные ошибки были пойманы тестами.
...
| Документохранилище и файлы | Загрузка, скачивание, типы файлов, ограничения вложений |
| Антивирусный контур файлов | Проверка безопасного сохранения, аудит-журнал без секретов, Telegram-просмотр последних проверок и заблокированных файлов |
| Мобильное приложение | Анализ кода, тесты экранов и совместимость с backend |
| Telegram-бот | Ключевые команды, e-mail, вложения, статусные проверки, request-layer |
...
После этих проверок файл проходит антивирусный контур Foreman. Для пользователя это выглядит просто: обычный чистый файл сохраняется как всегда, а файл с опасным или подозрительным результатом не попадает в документохранилище. Сам файл в таком случае не нужно пытаться открыть вручную: достаточно сообщить администратору, что загрузка была заблокирована.
