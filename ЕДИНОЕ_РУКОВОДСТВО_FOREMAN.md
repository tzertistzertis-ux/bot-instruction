...
Итого в `мегатесте-2` в текущем стандартном профиле проходит примерно 2090+ автопроверок, плюс отдельные backend/proxy regression/smoke-скрипты и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
| **Foreman сейчас** | **680-740** | Веб, мобильное приложение и Telegram-бот имеют несколько рабочих слоев защиты: серверные роли, гостевые ограничения, proxy-allowlist, антивирусный контур, контроль опасных форматов, журналы безопасности, управляемые Web Push-оповещения, регрессии, мегатест и мегатест-2. Но уровень снижен из-за отсутствия независимого пентеста, WAF/anti-DDoS, SSO/MFA, формального SOC/мониторинга и регулярного внешнего security-аудита |
...
|:---|:---|:---|:---|
| Foreman Web в браузере | 690-750 | Авторизация, серверные роли, защита гостя, лимиты скачивания, журнал безопасности, proxy-allowlist, антивирусный контур, запрет опасных вложений, профильные Web Push-подписки, админские правила push-событий, UI/visual-регрессии | Ужесточить CSP без `unsafe-inline`, добавить внешний WAF/anti-DDoS, SSO/MFA и независимый пентест |
...
| Система Foreman в целом | 680-740 | Защита не держится на одной кнопке: права проверяются сервером, файлы проходят антивирусный контур, лишние proxy-маршруты режутся до backend, важные события могут дублироваться управляемым Web Push, изменения проверяются тестовыми контурами | Внешний пентест, WAF/anti-DDoS, SSO/MFA, формальная карта реагирования на инциденты, регулярная проверка зависимостей и секретов |
...
### 1.15.4 Как создать пользователя аккуратно
