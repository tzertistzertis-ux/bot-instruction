...
| Backend-тесты антивируса и документохранилища Foreman | Политика `clean/no_verdict/api_unavailable`, полная отправка файла в Kaspersky, сохранение обычных файлов, `rtf` и строительных `dwg/dxf` с риск-меткой при неполном ответе Kaspersky, защита от старой серверной переменной форматов, серверный запрет скачивания без принятия риска, запрет прикрепления и e-mail-отправки рискованных файлов, запрет HTML Smuggling-файлов и маскировки HTML/JavaScript внутри `txt/doc` | 43 автотеста |
...
Итого в последнем успешном прогоне `мегатеста-2` проходит примерно 1991 автопроверка, плюс 3 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
