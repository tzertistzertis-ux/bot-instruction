...
| Регрессионное ядро | Backend, E2E/сквозные проверки, план-график, real-flow, типы заявок, мобильная совместимость, базовый smoke бота, серверная защита гостевого наблюдателя, базовая проверка proxy-allowlist | около 840+ автотестов в текущем составе smoke/regression-контуров |
...
| Тесты мобильного приложения | Экраны, маршруты, кэш заявок, заявки на заполнения/расходники, проверка безопасного обновления APK и совместимость с backend | 77 автотестов |

Итого в обычном мега-тесте проходит примерно 1050+ автопроверок, плюс сборка backend и статический анализ мобильного приложения. Число не высечено в камне: когда добавляются важные сценарии, тестов становится больше. Самым надежным источником точного числа всегда остается свежий вывод `pre_deploy_all.ps1`, `release_check.ps1`, `npm test` и `flutter test`.

Внутри регрессионного ядра проверки распределяются так:

| Подстадия<br>regression core | Количество |
|:---|:---|
| Jest-тесты `foreman-api` | 546 |
| Отдельные E2E/smoke-прогоны `foreman-api` | 15+ |
| Planning smoke | 28 |
| Real-flow smoke | 1 |
| Request-types smoke | 4 |
| Foreman Mobile compatibility smoke | 3 |
| Claim-bot release check smoke | 254 |
...
| Стадия<br>мега-теста | Что<br>проверяет | Примерный<br>объем |
|:---|:---|:---|
| Browser-тесты Foreman Web UI Playwright | Реальные действия в браузере: заявки, e-mail overlay, профиль, push-настройки, безопасное подключение MFA, план-график, retry при ошибке расписания | 17 браузерных автотестов |
...
| Backend-тесты bot-defense/honeypot и MFA-входа | Скрытое поле входа не видно человеку, один подозрительный случай только журналируется, временная блокировка включается осторожно, поддельные IP-заголовки не используются как доверенный адрес клиента, MFA не выдает полноценную сессию до кода, известное устройство хранится по hash, а риск входа пишется в audit без блокировки | 11 auth-security e2e-тестов плюс целевые unit-тесты нормализации IP, bot-defense, MFA, известных устройств и risk observations |
...
Итого в `мегатесте-2` в текущем стандартном профиле проходит примерно 2150+ автопроверок, плюс отдельные backend/proxy regression/smoke-скрипты и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
...
Отдельно проверено доверие к proxy-заголовкам. Foreman больше не строит блокировки, rate-limit и нормализованный `clientIp` по клиентским `X-Forwarded-For`, `X-Real-IP`, `CF-Connecting-IP` или `Forwarded`. Эти заголовки могут сохраняться только как сырая диагностика, если дошли до backend, но сами по себе не считаются доказательством адреса пользователя. Через публичный путь `claim-bot` такие клиентские forwarding-заголовки дополнительно отбрасываются на proxy-границе. На живой Amvera проверялся запрос входа с `X-Forwarded-For: 8.8.8.8` и `X-Real-IP: 9.9.9.9`: в bot-defense не записался подставленный адрес.
...
| Web cookie-auth | Веб-вход пытаются вернуть к хранению основного токена в `localStorage` или отдать секрет в ответе login | Тесты должны подтвердить, что веб-сессия держится через cookie и не раскрывает основной секрет странице |
| Доверие к IP | Поддельный `X-Forwarded-For` или `X-Real-IP` пытаются использовать как новый адрес для обхода rate-limit | Тесты должны показать, что блокировки строятся по нормализованному IP, а сырые заголовки остаются только диагностикой |
...
| Подтверждающие e-mail прорабам | В письмо пытаются одновременно вставить персональный и гостевой доступ, вернуть отгруженные заявки в мини-таймлайн или снять ограничение `max 10` | Тесты должны поймать такую ошибку |
...
```text
Mutation pilot summary: killed=37 survived=0 total=37
...
|:---|:---|
| Вход и безопасность | Неверный логин не дает 500, защита от подбора пароля, роли и права, HttpOnly cookie-сессия веба, CSRF-контур и защита rate-limit от поддельных forwarding-заголовков |
...
| Подтверждающие письма прорабам | Текст подтверждения, мини-таймлайн последних активных заявок с e-mail отправителя, отсутствие отгруженных заявок, лимит `max 10`, выбор персонального или гостевого доступа без дублирования |
...
| Proxy allowlist `claim-bot` -> Foreman API | Через публичный `claim-bot` проходят только заранее разрешенные рабочие API-маршруты; неизвестные или лишние backend-маршруты блокируются до обращения к `foreman-api`; клиентские forwarding-заголовки не передаются в backend как доверенные |
...
| 700-850 | Усиленный рабочий сервис: защита многослойная, есть реальные регрессии, ограничения, журналы и контроль опасных файлов |
| 850-950 | Корпоративный высокий уровень: дополнительно есть внешний пентест, WAF/anti-DDoS, зрелый мониторинг, формальная политика релизов и реагирования |
| 950-1000 | Уровень крупнейших технологических платформ: большие security-команды, bug bounty, постоянный SOC, независимые аудиты, формальные сертификации и огромный бюджет защиты |
...
| **Foreman сейчас** | **750-810** | Веб, мобильное приложение и Telegram-бот имеют несколько рабочих слоев защиты: серверные роли, гостевые ограничения, proxy-allowlist, антивирусный контур, контроль опасных форматов, журналы безопасности, осторожный honeypot/bot-defense на входе, HttpOnly cookie-сессию для веба, защиту rate-limit от простой подмены forwarding-заголовков, TOTP/MFA для веб-входа, известные устройства в audit-контуре, наблюдение риск-сигналов входа, управляемые Web Push-оповещения, регрессии, мегатест и мегатест-2. Диапазон повышен честно, но умеренно: MFA, cookie-сессия и защита от IP-spoofing закрыли практичные риски, но не заменили независимый пентест, WAF/anti-DDoS, SSO, формальный SOC/мониторинг и регулярный внешний security-аудит |
| Зрелая строительная SaaS/ECM-система | 820-900 | Обычно добавлены отдельные security-процессы, корпоративный мониторинг, внешние проверки, корпоративный SSO, централизованное управление MFA, договорные SLA и регулярные аудиты |
| Крупные платформы уровня Microsoft/Amazon/Яндекс | 930-990 | Огромные команды безопасности, круглосуточный мониторинг, независимые аудиты, программы поиска уязвимостей и большой бюджет на защиту |

Оценка Foreman по направлениям:

| Направление | Оценка | Что уже усиливает защиту | Что еще может поднять уровень |
|:---|:---|:---|:---|
| Foreman Web в браузере | 780-840 | Авторизация, серверные роли, защита гостя, лимиты скачивания, журнал безопасности, proxy-allowlist, honeypot/bot-defense на входе, HttpOnly cookie-сессия вместо хранения основного веб-токена в `localStorage`, нормализация IP для rate-limit, TOTP/MFA с QR-кодом и резервными кодами, выборочная админская галочка `Требовать MFA`, предупреждение перед подключением MFA, известные устройства по hash, audit-наблюдения риска входа, отключение наружного `X-Powered-By: Express`, антивирусный контур, запрет опасных вложений, профильные Web Push-подписки, админские правила push-событий, UI/visual-регрессии | Ужесточить CSP без `unsafe-inline`, добавить внешний WAF/anti-DDoS, SSO, включить risk-based step-up после периода наблюдения и провести независимый пентест |
| Telegram-бот и claim-bot | 710-770 | Проверка ролей, безопасные команды, quarantine/risk-gate файлов, ALARM-письма при вирусах, push по важным почтовым событиям, proxy-allowlist, отбрасывание клиентских forwarding-заголовков перед backend, диагностика, тесты полного regression-контура | Добавить более формальный мониторинг алертов, регулярный внешний аудит и отдельную карту реагирования на инциденты |
| Foreman Mobile | 700-760 | Актуальная APK-сборка в production работает только с утвержденным сервером Foreman, запрещает cleartext HTTP, очищает неподходящий сохраненный API-адрес, не использует дефолтный сервисный PIN, выпускается с постоянной production-подписью, нормальным applicationId `ru.tzertis.foreman`, отключенным backup и проверкой SHA-256 у скачиваемого APK | Провести независимый мобильный security-review, добавить отдельные контрольные проверки ручной публикации APK на Amvera, регулярно проверять зависимости и протестировать установку/обновление на нескольких реальных Android-устройствах |
| Система Foreman в целом | 750-810 | Защита не держится на одной кнопке: права проверяются сервером, файлы проходят антивирусный контур, лишние proxy-маршруты режутся до backend, веб-вход переведен на HttpOnly cookie, вход дополнительно прикрыт honeypot/bot-defense и TOTP/MFA, rate-limit не доверяет подставленным forwarding-заголовкам, известные устройства и риск входа пишутся в audit, важные события могут дублироваться управляемым Web Push, изменения проверяются тестовыми контурами | Внешний пентест, WAF/anti-DDoS, SSO, включение risk-based step-up после накопления наблюдений, формальная карта реагирования на инциденты, регулярная проверка зависимостей и секретов |

Главный вывод для руководства: Foreman не является “любительским сайтом с паролем”. По текущему набору защит он находится на уровне усиленного рабочего сервиса малого/среднего бизнеса, но пока не должен сравниваться со зрелыми промышленными платформами. Там безопасность держится не только на коде, но и на персонале с многоуровневыми служебными обязанностями. Одни подразделения круглосуточно анализируют события в IT-инфраструктуре, выявляют угрозы и оперативно реагируют на киберинциденты с привлечением внешнего аудита и поддержки bug bounty - программ, в рамках которых компании платят независимым исследователям, багхантерам или “белым хакерам” за обнаружение и подтверждение уязвимостей в продуктах, сервисах или инфраструктуре. Другие подразделения организуют физическую охрану серверных помещений, территории и контроль доступа к собственным ЦОД.
...
- Foreman видит знакомые веб-устройства по серверному hash, а не хранит открытый идентификатор устройства; эти данные не дают доступ сами по себе, а помогают аккуратнее оценивать вход;
- risk observations записывают факторы входа в audit-режиме: новое устройство, скорость отправки формы, неудачные попытки перед успешным входом, bot-defense и honeypot-сигналы. Пока это наблюдение, а не автоматическая блокировка по “цифровому слепку”;
...
- мобильная APK-сборка исключает увод пользователя на чужой сервер в production, имеет постоянную production-подпись, нормальный applicationId и проверяет SHA-256 скачиваемого обновления;
...
Дополнительно Foreman ведет audit-контур известных устройств и риск-сигналов входа. Устройство узнается по серверному hash, а не по открытому “цифровому слепку”. Эти данные не дают доступ сами по себе: они помогают понять, знакомый это браузер или новый, были ли странные попытки входа, нужно ли в будущем осторожно попросить дополнительный код. Сейчас риск-сигналы используются как наблюдение и журнал, а не как автоматический бан человека.
...
Актуальная Android-сборка Foreman Mobile выпускается не как временное debug-приложение, а как production APK с постоянной подписью и нормальным applicationId `ru.tzertis.foreman`.
...
Это не делает мобильное приложение “идеальным банковским уровнем”. Честный остаток риска остается: APK публикуется на Amvera вручную, поэтому важны аккуратное имя файлов, проверка манифеста обновления и контрольная установка на реальном устройстве после выпуска.

### Обновления APK

Мобильное приложение проверяет обновления через настроенный манифест. При скачивании APK приложение сверяет SHA-256 файла с тем значением, которое указано в манифесте обновления. Если файл поврежден, подменен или загружен не полностью, установка не должна продолжаться как обычное обновление.
...
### 1.15.6 Как создать пользователя аккуратно
...
Перед установкой Foreman Mobile сверяет контрольную сумму SHA-256 скачанного APK с манифестом обновления. Это нужно не для усложнения жизни пользователю, а чтобы поврежденный или подмененный файл не устанавливался как нормальная версия приложения.
