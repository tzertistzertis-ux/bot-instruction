...
| Полная регрессия Telegram-бота (`full`) | Команды, рендер, golden-проверки, старые сценарии, защита от регрессий, отдельная pytest-матрица реестра запусков | 1804 автотеста плюс render/golden-проверки |
...
Итого в последнем успешном прогоне `мегатеста-2` прошло примерно 1838 автопроверок, плюс 2 отдельных backend regression-скрипта и render/golden-проверки бота. Это не замена обычного мега-теста, а дополнительный слой для рискованных изменений.
