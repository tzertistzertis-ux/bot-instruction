...
Жесткое правило для любого исполнителя: перед Amvera сначала запускается
мега-тест `scripts\pre_deploy_all.ps1`. Если менялись auth/login, гостевой
доступ, права, лимиты, скачивание файлов, документохранилище, Foreman Web,
план-график, PDF/печать, mobile update, шаблоны, security, proxy, e-mail или
PostgreSQL, следом запускается `мегатест-2` через
`scripts\risk_assurance_all.ps1 -Profile standard`. Отдельные профильные
команды (`npm test`, `flutter test`, `release_check.ps1`, planning smoke,
visual update и т.п.) не заменяют эти gate. До зеленого результата `git push
amvera` запрещен.
...
| Предохранитель по IP | 3 одновременных и 30 скачиваний за 10 минут с одного IP | Один адрес не должен забирать весь гостевой лимит |
...
В обычное и повторное письмо Ларисе Разиповне добавляется короткая хронология и компактная HTML-схема: когда заявка пришла, когда Оксана Владимировна подтвердила запуск и что строка в Google-таблице на момент письма все еще не найдена.
