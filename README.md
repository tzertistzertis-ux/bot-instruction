...
- прораб говорит или пишет свободным языком — бот сам разбирает, что имелось в виду.
...
- определяет тип открывания, сторону петель, наличие импостов;
- запускает быстрый сбор черновика по балконному профилю без лишнего «ручного» ввода.

#### Как бот разбирает свободный текст: LLM-first + regex-fallback

Прорабы на стройке не пишут структурированные формы. Они пишут так, как думают:

> «ну там блок балконный он высотой 2300 а шириной 800 мм со створкой левой а и еще горизонтальный импост в нем на высоте где-то 1900 мм. да и еще оконный блок у него тоже есть слева он, а габариты его 1700 на 1900 мм, 1700 это высота. и заполнение везде — стеклопакет 4-10-4-10-4и»

Для человека из контекста всё понятно. Для программы — это хаос: габариты вперемешку с союзами, высота и ширина идут не в том порядке, импост упоминается между делом, а формула стеклопакета приклеена к букве «и».

Чтобы надёжно справляться с таким вводом, бот использует двухуровневую архитектуру парсинга:

**Первый уровень — YandexGPT (LLM).**
Если LLM включён (`YC_LLM_ENABLED=1`), бот отправляет текст прораба в YandexGPT с детальным системным промптом. Промпт объясняет модели:
- формат JSON-ответа с чёткой схемой (`width`, `height`, `opening`, `side`, `impost_type`, `impost_pos`, `glass_formula`);
- 10 правил извлечения (какие поля обязательны, как отличить балконную дверь от окна, что делать с неявными значениями);
- конкретный пример входа и ожидаемого выхода.

Модель возвращает структурированный JSON, который бот нормализует теми же валидаторами, что и regex-парсер: проверяет диапазоны габаритов, приводит тип открывания к каноническим значениям, валидирует позицию импоста.

**Второй уровень — regex-fallback.**
Если LLM недоступен, отключён или вернул ошибку, бот молча переключается на детерминированный regex-парсер. Этот парсер работал и раньше — он по-прежнему надёжно разбирает структурированные фразы вроде `2200x800 пов-откидная левая MACO`.

Мини-кейс:
- Прораб голосом: `заявка в альтавин: балконный блок высота 2300 ширина 800, створка левая поворотно-откидная, импост горизонтальный на 1900, стеклопакет 4-10-4-10-4и, фурнитура MACO, и окно слева 1700 на 1900`
- YandexGPT разбирает на два блока: балконная дверь и оконный блок, каждый со своими габаритами и параметрами.
- Бот нормализует, валидирует и показывает черновик с пометкой `(парсер: LLM)`.
- Если бы LLM был недоступен, regex-парсер обработал бы ту же фразу с пометкой `(парсер: regex)`.

Под капотом:
- LLM-вызов обёрнут в `asyncio.to_thread` + `asyncio.wait_for` с таймаутом, чтобы не блокировать основной event loop и не «подвесить» бота при медленном ответе API;
- нормализация LLM-ответа проходит через те же валидаторы, что и regex, поэтому бот не принимает от модели «мусорные» данные;
- в ответе бота видно, какой парсер сработал: `(парсер: LLM)` или `(парсер: regex)` — это помогает при отладке и контроле качества.
...
YandexGPT в текущем коде включен **не глобально на все команды**, а в трёх контролируемых контурах:

- `LLM_SCOPE: consumables_parser + safe_intent_router + altavin_parser` (это видно и в `/status`);
...
- **альтавин-парсер** (новый контур): `altavin_analytic.py` — разбор свободного текста прораба в структурированные блоки оконно-балконных конструкций.
...
- если LLM отключен ENV-флагами — для всех трёх контуров.
...
- по названию объекта: `найди заявки прайм`, `покажи заявки по прайму`, `заявки по объекту сенат`
...
- `покажи ЖА-43 и перешли файл Клочкову`
...
## 2.16 Многосоставные команды и планировщик блок-связок

Бот умеет разбирать «одну длинную фразу» как набор отдельных действий — без дополнительных кнопок и лишних уточнений.

### Как это работает

Если в сообщении встречается два или три глагола-действия (`покажи`, `найди`, `перешли`, `отгрузи` и т.п.), планировщик (`multi_action_planner.py`) автоматически разбивает фразу на отдельные шаги — «блок-связки» — и выстраивает их в очередь выполнения.

Мини-кейс:
- Пользователь пишет: `покажи ЖА-43 и перешли файл Клочкову`
- Бот понимает: два действия — сначала открыть карточку ЖА-43, потом переслать файл Клочкову.
...
- Планировщик ищет глаголы-триггеры (`покажи`, `найди`, `перешли`, `перекинь`, `вышли`, `скинь`, `отгрузи`) и разбивает фразу по точкам входа.
- Каждому сегменту назначается действие (`open_claim_card`, `find_claim_files`, `start_forward_flow`, `start_shipment_flow`, `show_my_recent_claims`).
...
- `открой ЖА-43 и перешли файл Клочкову`
- `жа43 открой и перекинь файл Иванову`
- `покажи мои последние заявки, найди файлы по ЖА-43, запросить отгрузку ЖА-43`

---

## 2.17 Личное общение между сотрудниками
...
1. Прораб пишет: `открой ЖА-43 и сразу перешли файл Клочкову`
...
5. Бот последовательно открывает карточку и запускает мастер пересылки на Клочкова.
...
7. Прораб выбирает Клочкова; бот уточняет: Telegram или e-mail.
8. Файл уходит напрямую в личный кабинет Клочкова — без попадания в общий чат.
...
- `multi_action_planner.py` — разбор составных фраз: выделяет глаголы-триггеры, строит очередь блок-связок, наследует номер заявки между шагами, отбрасывает временну́ю лексику и информационный мусор.
