...
- По коду `command_delete.py` восстановление удалённых файлов разрешено только спец-админу (`Лапкин А.О`).
...
3. оконный блок с импостами — **план (в разработке)**
4. создать балконный блок — **активно**
...
### Что обновлено сегодня в UX балконного модуля

В `command_window_balcony.py` обновлена подача шагов, чтобы прорабу было легче читать с телефона:

- В экране проверки/сохранения заголовок конструкции (`Блок #...`) выделяется жирным.
- Меню `Редактирование заполнения полей` сокращено и очищено от перегруженных формулировок.
- Пункты выбора поля теперь привязаны к блоку (`... в блоке #N`), а не к длинным техническим строкам.

Пример (2 поля):

```text
Редактирование заполнения полей:
Выберите поле для правки:
1 - левая секция в блоке #2
2 - правая секция (створка) в блоке #2
0 - назад в меню правки
```
...
Выберите поле для правки:
1 - левая секция в блоке #2
2 - правая секция (створка, верх) в блоке #2
3 - правая секция (створка, низ) в блоке #2
0 - назад в меню правки
```
...
Редактирование заполнения полей:
Выберите поле для правки:
1 - верхняя левая секция в блоке #2
2 - верхняя правая секция в блоке #2
3 - нижняя левая секция в блоке #2
4 - нижняя правая секция в блоке #2
0 - назад в меню правки
```
...
### 3.9 Сценарий «отправил ...» и вечерний контроль

1. Пользователь пишет: `отправил ЖА-34`.
2. Номер попадает в `sent_claims`.
3. По расписанию бот сверяет фактическое наличие в таблице.
4. Если не найдено, бот поднимает тревогу в чат.

### 3.10 Вечерний отчет 18:00 (реестр + незаведенные)

1. В 18:00 бот отправляет сводку:
   - что заведено сегодня в таблицу;
   - что еще не заведено из отправленных.
2. Раздел «не заведены» строится сравнением:
   - внутреннего списка `sent_claims`,
   - и фактических номеров в Google Sheets.

### 3.11 Защита от «дублирующихся» сообщений Telegram
...
## 4. Архитектура (граф модулей)

```mermaid
flowchart TD
    U[Пользователь] --> BOT[bot.py]
    BOT --> TH[telegram_handlers.py]
    BOT --> JOBS[jobs.py]

    TH --> HELP[help_menu.py]
    TH --> SHOW[command_show.py]
    TH --> FILECMD[command_files.py]
    TH --> CARD[command_claim_card.py]
    TH --> DEL[command_delete.py]
    TH --> FWD[command_forward.py]
    TH --> SHIP[command_shipment.py]
    TH --> AREP[admin_report.py]
    TH --> ROOT[command_constructor.py]

    ROOT --> FILL[command_fill_claim.py]
    ROOT --> CONS[command_consumables.py]
    ROOT --> WIN[command_window.py]
    WIN --> BAL[command_window_balcony.py]
    WIN --> WEDIT[command_window_edit.py]
    WIN --> WHELP[command_window_helpers.py]

    FILL --> RF[render_fill_claim_svg.py]
    CONS --> RC[render_consumables_svg.py]
    BAL --> RB[render_balcony_svg.py]
    WHELP --> RW[render_svg.py]

    TH --> FILES[files.py]
    TH --> SHEETS[sheets.py]
    TH --> MAIL[mail_utils.py]
    TH --> PF[profanity_filter.py]
    TH --> AUDQ[audio queue/worker]
    AUDQ --> ATRANS[audio_transcribe.py]
    ATRANS --> ASTTV3[yandex_stt_v3_streaming.py]
    TH --> ACH[audio_chunking.py]
    TH --> LLP[yandex_llm_parser.py]
    TH --> DEDUP[update_dedup.py]
    TH --> WATCH[dialog_watchdog.py]
    TH --> WSTATE[wizard_state_store.py]
```

## 4.1 Полный древовидный граф взаимодействия (вертикально, сверху вниз)

Чтобы граф было удобно листать на телефоне и ноутбуке, полный контур дан в 2 вертикальных частях.

### 4.1.A Маршрутизация, команды, конструкторы
...
### 4.1.B Хранение, защита, интеграции, фоновые задачи

```mermaid
flowchart TD
    TH[telegram_handlers.py] --> ATT[attachment_guard.py]
    TH --> PF[profanity_filter.py]
    TH --> DEDUP[update_dedup.py]
    TH --> WD[dialog_watchdog.py]
    TH --> WSS[wizard_state_store.py]
    TH --> AUD[claim_audit.py]
    TH --> MET[metrics.py]
    TH --> FILES[files.py]
    TH --> SHT[sheets.py]
    TH --> MU[mail_utils.py]
    TH --> IDX[file_index.py]
    TH --> PREV[preview_spool.py]

    FILES --> FS[(Файловое хранилище /data)]
    IDX --> SQLITE[(SQLite file_index)]
    WSS --> WJSON[wizard_states.json]
    PREV --> PCACHE[(preview_cache /data)]

    SHT --> GS[Google Sheets: Учет заявок]
    MU --> SMTP[SMTP почта]
    MU --> IMAP[IMAP почта]
...
- `storage.py`:
  - `received_claims.json` — какие заявки уже получили файл;
  - `pending_reminders.json` — кому уже отправлено напоминание;
  - `sent_claims.json` — контроль «отправил ...» и база для отчёта «не заведены в таблицу»;
...
- Фоновый job `check_new_claims_and_remind` сравнивает реестр заявок и маркер получения файла `received_claims.json`.
- Если заявка появилась в «Учёт заявок», но маркер получения файла не установлен:
  - уходит первое e-mail напоминание ответственному;
  - если спустя 48 часов маркер всё ещё не появился, уходит повторное напоминание.
...
- Отчет строится в двух частях:
  - что заведено сегодня в таблицу;
  - что не заведено из списка `sent_claims`.
- Логика «не заведены»:
  - бот берет внутренний список «отправленных»;
  - сверяет с фактическими номерами в Google Sheets;
  - выводит только реальную разницу.
- Это исключает «вымышленные номера»: источник всегда либо таблица, либо внутренний журнал отправки.
