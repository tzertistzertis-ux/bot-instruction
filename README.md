...
Мини-кейс:
- Пользователь пишет: `меню`.
- В личном кабинете бот отвечает коротко и по делу: `Выберите раздел кнопками.`
- Сразу под сообщением появляется корневое меню с крупными кнопками разделов.
- В групповом чате бот поддерживает отдельное служебное сообщение с теми же разделами, чтобы рабочие отчёты и уведомления не перемешивались с навигацией.
...
- Корневое меню теперь работает в логике `кнопка сначала, текст — как запасной путь`.
- В личке корень рисуется как компактное сообщение плюс `ReplyKeyboard` с разделами.
- В группе бот держит отдельное служебное сообщение с `InlineKeyboard`, а рабочие отчёты присылает уже отдельными сообщениями ниже.
- Старые прямые команды и даже цифровые ответы не выброшены: они по-прежнему принимаются как совместимый fallback.

Корневые разделы, которые пользователь видит на кнопках сейчас:
- `ПОИСК ЗАЯВОК`
- `ТАБЛИЦА УЧЕТА`
- `СОЗДАНИЕ ЗАЯВОК`
- `ПРАВКА И ОТПРАВКА`
...
- `ЖУРНАЛ`
- `РЕЕСТР ЗАПУСКОВ` / `РЕЕСТР ЗАПУСКОВ (ВЫКЛ.)`
- `ГОЛОС`
- `ШАБЛОНЫ ДЛЯ ЗАМЕРОВ`
- `ВЫХОД`
...
Сейчас у бота две навигационные линии, и это удобно в живой работе:
- Постоянная нижняя клавиатура под полем ввода.
- Отдельные кнопочные экраны разделов.

Постоянная нижняя клавиатура (`ReplyKeyboardMarkup`, `is_persistent=True`) такая:
- В общем чате: `Меню`, `Личный кабинет`.
- В личном кабинете: `Меню`, `Общий кабинет`.
...
- После этого команда `меню` в личке больше не раскрывает длинную текстовую таблицу, а показывает короткий ответ и сразу даёт крупные кнопки разделов.
...
- В общем чате поверх нижней клавиатуры бот поддерживает отдельное служебное сообщение: `Бот работает с помощью интерактивных кнопок.` Именно там живёт корневое inline-меню для группы.
...
### `/status` и health-check endpoints
...
- Для внешней HTTP-проверки контейнера используйте прежде всего `GET /healthz` и `GET /readyz`.
- В коде есть совместимость с `GET /health` и `GET /ready`, но для реального smoke-check на домене надёжнее опираться именно на `healthz`.
...
Что делает:
- мягко восстанавливает навигацию без перезапуска диалога;
- в группе обновляет постоянную нижнюю клавиатуру и при необходимости пересобирает служебное меню с inline-кнопками;
- в личке заново показывает корневые кнопки разделов и нижнюю навигацию.
...
### Как сейчас устроен раздел с кнопками

Раздел `ПОИСК ЗАЯВОК` больше не выглядит как сухая памятка с цифрами. Теперь это рабочий кнопочный экран, который ведёт человека от общего запроса к нужной заявке без лишнего вспоминания команд.

Основные кнопки раздела такие:
- `ОДНА ЗАЯВКА`
- `ГРУППА ЗАЯВОК`
- `ПО НОМЕРУ ЗАКАЗА`
- `ПО ОБЪЕКТУ`
...
Что важно для повседневной работы:
- `ПО ОБЪЕКТУ` и `ПО ФИО` умеют показывать кнопками живой топ-список из текущих данных.
- После выбора объекта или составителя бот предлагает количество: `10`, `20`, `50` или принимает своё число текстом.
- `ЕЩЁ ФИЛЬТРЫ` сейчас ведёт к трём рабочим кнопкам: `НЕЗАПУЩЕННЫЕ`, `НЕ ЗАКАЗАННЫЕ`, `НА БАЗЕ`.
- Любой список заявок в этом разделе теперь нумеруется.
- После списка бот пишет понятную подсказку: если нужна полная карточка, введите номер строки из списка и нажмите `↵`.

Старые текстовые команды не выкинуты. Если человеку привычнее написать руками `покажи заявки жа` или `покажи заявки по прайму`, бот это тоже поймёт.
...
Под капотом:
- Из номера заказа удаляются все нечисловые символы: `С-26-00314` → `2600314`, а короткие формы вроде `С-148` нормализуются до хвоста номера.
- Такой же «чистый» вид формируется для каждой строки в таблице учёта.
- Совпадение ищется как точное, так и по окончанию номера, поэтому короткие формы с префиксом тоже работают.
...
Мини-кейс:
- Пользователь нажимает `ПО ОБЪЕКТУ`, выбирает, например, `ГОРОД ЗВЕЗД`, а затем нажимает `10`.
- Бот пишет, сколько заявок найдено по объекту, и показывает нумерованный список последних заявок.
- Если потом ввести, например, `3`, бот откроет полную карточку третьей заявки из этого списка.
...
- Для кнопочного режима бот строит динамический топ объектов по текущему снапшоту и показывает его страницами по 8 кнопок: с `ЕЩЁ`, `◀ ПРЕД`, `▶ СЛЕД` и `НАЙТИ ВРУЧНУЮ`.
...
Мини-кейс:
- Пользователь нажимает `ПО ФИО`, выбирает нужного составителя кнопкой или вводит фамилию вручную.
- Бот показывает найденные заявки нумерованным списком.
- Если фамилия неоднозначная, бот не притворяется всезнайкой, а честно раскладывает результат по людям.
...
- В кнопочном режиме бот сначала показывает топ составителей из живых данных, а затем при необходимости даёт ручной ввод и постраничный просмотр списка.
...
- Бот показывает 5 последних заявок префикса `ЖА` нумерованным списком.
- После этого можно ввести номер строки, и бот откроет подробную карточку выбранной заявки.
...
- Список также нумеруется, поэтому не нужно переписывать номер заявки вручную: достаточно ввести номер строки.
...
- Если включён `LAUNCH_REGISTRY_ENABLED=1`, на корневой кнопке есть раздел `РЕЕСТР ЗАПУСКОВ`.
- Если флаг выключен (`0`), пользователь увидит `РЕЕСТР ЗАПУСКОВ (ВЫКЛ.)`, а бот честно скажет, что раздел временно недоступен.
- Внутри раздела сейчас работают кнопки:
  - `ПОИСК ПО ЗАПУСКУ`
  - `ЗАПУСКИ ПО ОБЪЕКТУ`
  - `ЗАПУСКИ ЗА ПЕРИОД`
  - `ИСТОРИЯ ПО ОБЪЕКТУ`
...
1. Пользователь нажимает `РЕЕСТР ЗАПУСКОВ`.
2. Выбирает нужное направление кнопкой.
3. Бот даёт короткую подсказку и ждёт один понятный запрос в текущем чате.

Есть ещё отдельный быстрый сценарий после годового меню объектов:
- если в блоке `Готов сформировать файлы...` выбрать `2. НЕТ`, бот открывает вспомогательное меню:
  - `ПОИСК ПО ОБЪЕКТУ`
  - `ИСТОРИЯ ОБЪЕКТА`
  - `ПОИСК ЗАПУСКА`
  - `ПОИСК ПО № ЗАКАЗА`
...
- `реестр 00148`, `реестр С-148`, `реестр С148`, `реестр С 148` — короткие формы этого же поиска;
...
- Если пользователь идёт по кнопочному сценарию, бот сначала даёт короткую подсказку, а уже потом принимает сам запрос в этом же чате, без отдельного “мастера в десять шагов”.
...
Раздел 10 больше не похож на старый “мастер с одной строкой ввода”. У него теперь два рабочих лица: основной контур в Telegram WebApp и текстовый fallback, который остаётся на случай совместимости, аварийного обхода и быстрых точечных правок. Это важный сдвиг: пользователь меняет форму взаимодействия, а не саму систему. И WebApp, и текстовый режим приходят к одному и тому же `workbook`, одному `template.measure.json` и одному комплекту итоговых файлов.

### Из чего теперь собран раздел

Живой технический маршрут выглядит так:
1. `help_menu.py` и кнопка `Web App` открывают веб-интерфейс прямо внутри Telegram.
2. `bot_modules/health_http.py` обслуживает не только `/healthz`, но и WebApp-маршруты `/webapp/measure` и `/api/webapp/*`.
3. `bot_modules/webapp_measurements.py` проверяет `initData` Telegram, поднимает сессию, открывает каталог шаблонов, держит ACL, блокировки, журнал действий и e-mail API.
4. `assets/webapp_measure/app.js` и `assets/webapp_measure/styles.css` рисуют саму рабочую поверхность: splash-экран, стартовую страницу, picker, карточку листа, review-модалку и e-mail-модалку.
5. `measurement_template_workbook.py` хранит фактические значения, прогресс листа и состояние типовых этажей.
6. `measurement_template_calibration.py` проверяет корректность `template.measure.json`.
7. `measurement_template_fill.py` и `measurement_template_pdf.py` собирают итоговые наложения на `PNG/PDF`.
8. `command_measurement_templates.py` остаётся живым fallback-контуром: он нужен для текстового режима, для совместимости со старыми сценариями и для части сервисных handoff-потоков.

Ключевая мысль здесь простая: в проде всё начинается не с `DXF`, а с уже подготовленного runtime-комплекта листа. Бот и WebApp читают `template.measure.json`, `template_base.pdf`, `template_base.png`, `preview.png` и, при необходимости, `template.svg` как legacy-резерв. Список листов в каталоге строится только по имени `template.measure.json`; прочие `*.measure.json` рядом с ним считаются служебными и в меню не попадают.

### Что видит пользователь в WebApp

WebApp сделан не как “сайт рядом с ботом”, а как продолжение замерного сценария:
1. при первом открытии в сессии пользователь видит короткую silent-заставку;
2. стартовый экран показывает “Продолжить работу”, “Выбрать объект”, “Активные шаблоны”, “Назад в бот”;
3. в блоке незавершённой работы видны карточки листов с прогрессом, статусом и последним редактором;
4. кнопка открытия шаблона защищена двойным нажатием, чтобы случайный тап в дороге не уводил человека в другой лист;
5. кнопка `E-Mail` сразу подсказывает состояние листа: незавершённый лист подсвечивается как pending, полностью образмеренный — как ready;
6. picker позволяет пройти по объекту, секции, этажу и листу без “лестницы” из десятка сообщений;
7. внутри листа пользователь работает по самому чертежу: зум, панорамирование, выбор проёма, правка размеров, применение к семейству, `Просмотр размеров`, отправка на e-mail.

Технически важно, что WebApp не рендерит лист из воздуха. Он показывает заранее собранный `preview.png` или базовый `template_base.png`, а фактические значения кладёт в тот же `workbook`, что и текстовый сценарий. Поэтому у системы нет двух конкурирующих правд: есть один источник данных и два удобных способа до него добраться.

### Почему WebApp оказался важнее, чем кажется

Пользователь, далёкий от кода, замечает только одно: лист перестал распадаться на десятки сообщений. Но под капотом выигрыш гораздо шире:
- у листа появился понятный прогресс: сколько проёмов заполнено и сколько ещё ждут внимания;
- видно, кто последним открывал и редактировал шаблон;
- блокировки не дают двум прорабам молча перетирать один и тот же лист;
- review-модалка и e-mail-модалка работают в том же контексте, без “прыжков” между экранами;
- список адресатов в e-mail-модалке сохраняет позицию прокрутки при выборе, отправке и показе ошибки, что особенно важно на длинных списках в мобильном Telegram.

Иными словами, WebApp здесь решает не эстетическую задачу, а производственную: удерживает внимание пользователя на листе и уменьшает число случайных ошибок в конце длинного рабочего дня.

### Текстовый fallback никуда не делся

Старый текстовый маршрут по-прежнему жив и нужен не “для истории”, а как сервисный запасной выход. Он полезен, когда:
- у пользователя нет нормального WebView;
- нужно быстро добить один-два размера без открытия полного интерфейса;
- идёт диагностика или восстановление после нестандартного сбоя.

Входные формулировки остаются прежними:
- `шаблоны для замеров`
- `меню замеров`
- `замеры`
- `замеры этажа`
...
И дальше бот всё так же понимает живой рабочий ввод.
...
Текстовый сценарий по-прежнему мягко нормализует маркировки. Для пользователя это выглядит почти магически, а для системы означает, что `ОК-1`, `ОК1`, `ОК 1`, `ОК-1пр**` и похожие хвосты не разваливают одну и ту же габаритную семью на пять “разных” команд.

### Правило безопасности: массовый ввод не должен тихо ломать готовую работу

Команда вида `ОК-1 2130 830` применяется только к ещё не запущенным проёмам. Уже сохранённые позиции не перетираются молча. Если все подходящие элементы уже были заполнены, бот честно говорит, что ничего не менял. Это одно из тех маленьких правил, которые редко замечают до первого инцидента и очень ценят после него.

### Типовые этажи и многоблочные листы

Контур типовых этажей никуда не исчез:
1. пользователь выбирает шаблон вроде `3-11 этажи / лист 1`;
2. система просит фактический этаж, например `4`;
3. если у соседнего этажа уже есть значения, предлагает аккуратно перенести их в текущий `workbook`;
4. в финале подставляет в шапке реальный `ЭТАЖ № N`, а не абстрактный диапазон.

Для многоблочных проёмов логика тоже общая для обоих интерфейсов: позиция считается заполненной только тогда, когда в ней закрыты все обязательные блоки. Поэтому команда `7 б2 1400 820` правит конкретный блок, а не “примерно что-то у седьмой конструкции”.

### Что выходит на финише

После завершения работы пользователь получает не “ещё одно промежуточное состояние”, а рабочий пакет:
- итоговый `PNG`;
- итоговый `PDF`;
- запись в `мои последние файлы`;
- возможность сразу же отправить комплект на e-mail из WebApp или через старый delivery-сценарий.

Для почтового сценария WebApp поддерживает оба живых источника адресатов:
- выбор из списка известных контактов;
- ручной ввод e-mail прямо в модалке.

Именно здесь становится видно, зачем раздел 10 построен на runtime-артефактах, а не на DXF: боевой контур должен быстро и устойчиво выдавать `PDF/PNG`, а не заново пересобирать инженерный исходник на каждом пользовательском действии.

### Как вернуться позже

Возврат теперь тоже двусторонний:
- `Web App` и экран “Активные шаблоны” возвращают в живой лист;
- `мои последние файлы` быстро дают последнюю актуальную версию уже собранного `PDF/PNG`;
- текстовые команды `список`, `сброс`, `сформируй`, `нн`, `0` остаются как короткий сервисный язык раздела.

### Короткая памятка по самым полезным действиям

- `Web App` — основной вход в современный замерный интерфейс.
- `шаблоны для замеров` — гарантированный вход в раздел, если нужно открыть fallback-сценарий.
- `ОК-1 2130 830` — массово назначить размеры габаритной группе.
- `1,2 2200 800` — дать одну пару размеров нескольким пунктам.
- `4 2145 845` — точечно исправить позицию по номеру.
- `RP... 2145 845` — точечно исправить позицию по спецмаркировке.
- `7 2200 700 1400 800` — заполнить многоблочную конструкцию.
- `7 б2 1400 820` — исправить только один блок.
- `сброс` — очистить текущий лист.
- `сформируй` — собрать итоговые `PDF/PNG` в fallback-сценарии.
...
- по номеру заказа: `С26-00276`, `С-26-00310`, `С-148`, `С148`, `С 148` (номер заказа из офиса)
...
- основной рабочий путь: кнопка `Web App` в Telegram. Она открывает интерфейс с объектами, активными листами, zoom/pan, review-модалкой и e-mail-модалкой
- текстовый fallback сохранён для совместимости и быстрых правок, он работает поверх того же `workbook` и тех же runtime-шаблонов
- массово по группе:
  - `ОК-1 2130 830`
  - `ОК1 2130х830`
  - `ОК 1 2130 830`
...
- сервис внутри шаблона:
  ...
  - `нн`
  - `0`
- важное отличие WebApp: прогресс листа, блокировки, последний редактор и отправка на e-mail теперь видны прямо в карточке шаблона, без длинной переписки по шагам
...
```mermaid
flowchart TD
    U[Пользователь] --> TGC[Telegram клиент]
    TGC --> TGA[Telegram Bot API]
    TGC --> WA[Telegram WebApp]
    TGA --> BOT[bot.py]

    BOT --> TH[telegram_handlers.py]
    BOT --> JOBS[jobs.py]
    BOT --> HC[healthcheck.py]
    BOT --> HHTTP[bot_modules/health_http.py]
    BOT --> CFG[config.py]
    ...
    HM --> LR
    HM --> MT
    HM -->|кнопка Web App| WA
    WA -->|GET /webapp/measure| HHTTP
    WA -->|web_app_data| TH
    HHTTP -->|/webapp/measure + /api/webapp/*| WAPP[webapp_measurements.py]
    WAPP --> WAF[assets/webapp_measure/app.js + styles.css]

    MT --> MTWB[measurement_template_workbook.py]
    MT --> MTFILL[measurement_template_fill.py]
    MT --> MTPDF[measurement_template_pdf.py]
...
- `telegram_handlers.py` остаётся главным диспетчером текстовых команд, документов, callback-кнопок и `web_app_data`;
- WebApp-контур теперь входит в архитектуру официально: `health_http.py` обслуживает WebApp-маршруты, а `webapp_measurements.py` стал отдельным HTTP/API-слоем для замеров;
- `command_measurement_templates.py` не исчез, а превратился в text fallback и сервисный вспомогательный слой раздела 10;
- `measurement_template_workbook.py`, `measurement_template_fill.py`, `measurement_template_pdf.py` и `measurement_template_calibration.py` стали общим “сердцем” замерного контура для обоих интерфейсов;
- конструкторы, поиск, карточки, почта и фоновые задачи по-прежнему собираются вокруг `files.py`, `file_index.py`, `sheets.py`, `mail_utils.py` и `jobs.py`.
...
flowchart TD
    ...
    TH -->|шаблоны / замеры / text fallback| MT[command_measurement_templates.py]
    TH -->|/status health-фрагмент| LRA[launch_registry_admin.py]
    HM -->|активация раздела 8| LR
    HM -->|активация раздела 10| MT
    HM -->|кнопка Web App| WA
    WA -->|HTTP GET/POST| HHTTP
    WA -->|web_app_data| TH
    HHTTP -->|/webapp/measure + /api/webapp/*| WAPP[webapp_measurements.py]
    WAPP --> WAF[assets/webapp_measure/app.js + styles.css]
    HHTTP -->|HTTP probes| HC
    PM --> FW
...
flowchart TD
    ...
    MT[command_measurement_templates.py] -->|text fallback| MTWB[measurement_template_workbook.py]
    MT --> MTCAL[measurement_template_calibration.py]
    MT --> MTFILL[measurement_template_fill.py]
    MT --> MTPDF[measurement_template_pdf.py]
    MT --> FILES2[files.py]
    ...
    HHTTP[bot_modules/health_http.py] --> WAPP[webapp_measurements.py]
    WAPP --> WAF[assets/webapp_measure/app.js + styles.css]
    WAPP --> MTWB
    WAPP --> MTCAL
    WAPP --> MTFILL
    ...
    WAPP --> FILES2
    WAPP --> MU3
    WAPP -->|build output bundle for e-mail| MT
...
flowchart TD
    ...
    WAPP --> FILES
    WAPP --> MU
    WAPP --> MTWB[measurement_template_workbook.py]
    WAPP --> MTCAL[measurement_template_calibration.py]
    WAPP --> WACL[(measurement_webapp_acl.json)]
    WAPP --> WASESS[(WebApp sessions + locks)]
    ...
    FILES --> FS[(Файловое хранилище /data, /data/templates, /data/telegram_user_files, /data/email_claim_files)]
...
- `telegram_handlers.py` по-прежнему раздаёт задачи по профильным модулям, но теперь рядом с ним официально живёт и WebApp-вход;
- `health_http.py` больше нельзя читать только как health-server: это ещё и HTTP-шлюз замерного WebApp;
- раздел замеров теперь имеет два входа в один и тот же движок: `command_measurement_templates.py` как text fallback и `webapp_measurements.py` как основной WebApp/API-контур;
- `measurement_template_workbook.py`, `measurement_template_calibration.py`, `measurement_template_fill.py` и `measurement_template_pdf.py` — это общий вычислительный слой, а не “принадлежность” только старого текстового сценария;
- `webapp_measurements.py` добавил к замерному контуру то, чего раньше не было в явном виде: сессии, ACL, блокировки, журнал и мобильный UI на `assets/webapp_measure/*`;
- файловый, почтовый и замерный контуры теперь можно читать отдельно, не теряя общей картины.
...
- `telegram_handlers.py` — центральный роутер всех сообщений, документов, callback-кнопок и переходов между общим чатом, личкой и кнопочными разделами.
- `config.py` — все ENV и бизнес-ограничения.
- `help_menu.py` — кнопочное меню разделов, тексты подсказок, state-машина переходов и fallback на старые текстовые команды.
- `group_service_menu.py` — постоянное служебное inline-меню в групповом чате, хранение `message_id` и восстановление меню после удаления или обновления.
- `health_http.py` — встроенный HTTP health-server для `/healthz`, `/readyz` и совместимых health-alias.
- `command_launch_registry.py` — роутер раздела `8` (`Реестр запусков`): вход, выход, safe-search режим.
- `command_measurement_templates.py` — fallback-роутер раздела `10` (`Шаблоны для замеров`): текстовый сценарий, сервисные переходы и совместимость со старыми командами.
- `webapp_measurements.py` — основной HTTP/WebApp-слой раздела `10`: Telegram `initData`, `/webapp/measure`, `/api/webapp/*`, ACL, блокировки, журнал, e-mail API, выдача каталога и карточек листов.
- `assets/webapp_measure/app.js` — фронтенд WebApp замеров: silent splash, стартовый экран, picker, zoom/pan, sheet-editor, review и e-mail модалки.
- `assets/webapp_measure/styles.css` — мобильная стилизация WebApp, safe-area, фиксированные overlay и визуальные статусы карточек/кнопок.
- `launch_registry_admin.py` — добавляет в `/status` диагностический блок состояния реестра запусков.
...
- `command_show.py` — поиск заявок (по номеру, префиксу, номеру заказа, названию объекта со стеммингом, составителю), нумерованные списки с выбором карточки по номеру строки, таблица, журнал действий.
...
- `measurement_template_pdf.py` — точная сборка `PDF` по базовому `template_base.pdf`, placement-геометрии и текстовым наложениям без возврата в DXF-контур.
