...
- `7` — Журнал действий
- `8` — `Реестр запусков` (если `LAUNCH_REGISTRY_ENABLED=1`) / `Dry-run (без изменений)` (если `LAUNCH_REGISTRY_ENABLED=0`)
...
Раздел 8 включается флагом `LAUNCH_REGISTRY_ENABLED`.
Если флаг выключен, пункт 8 в меню остаётся старым dry-run сценарием отправки.

Как пользователь входит в раздел 8:
1. Открывает `справка/меню`.
2. Выбирает раздел `8`.
3. Отправляет явный триггер входа, например: `проверь отправку ЖА-34`.
4. Бот отвечает: `Раздел «Реестр запусков» активирован...`.

Что можно писать после входа (неявный режим внутри раздела):
- `84`
- `Универ`
- `С-26-00307`
- `сводка Универ`
...
Что можно писать вне раздела 8 (только явные префиксы):
- `реестр ...`
- `запуск ...`
- `сводка ...`
- `история ...`
- `реестр статус` / `статус реестра`

Выход из раздела:
- `0`, `назад`, `отмена`, `выход`, `стоп`, `закрыть`.

Важно по безопасности маршрутизации:
- без явного входа в раздел 8 короткие запросы вроде `84`, `Универ`, `С-26-00307` не должны перехватываться роутером реестра;
- это защищает старые сценарии поиска заявок и карточек от ложных срабатываний.

Как устроен импорт с почты:
- отдельный mail-poller не запускается;
- импорт встроен в существующий `check_mail` job;
- при подходящем письме с JSON-вложением обновляется SQLite-база реестра (`LAUNCH_REGISTRY_DB_PATH`), а сырой JSON сохраняется в `LAUNCH_REGISTRY_RAW_DIR`.
...
- вход из меню: `8` -> явный триггер `проверь отправку ЖА-34`
- внутри раздела можно писать без префикса: `84`, `Универ`, `С-26-00307`, `сводка Универ`, `история Универ`
- вне раздела безопасные явные команды: `реестр ...`, `запуск ...`, `сводка ...`, `история ...`
...
- `LAUNCH_REGISTRY_IMPORT_SUBJECT_HINT` — обязательная фраза в теме письма.
  Для вашего сценария: `АБЛИЦА_запусков_Автосохраненный_`.
